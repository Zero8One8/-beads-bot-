...
async def get_categories_keyboard():
    ...
    # Добавляю браслеты и корзину
    buttons.append([types.InlineKeyboardButton(text="💎 БРАСЛЕТЫ", callback_data="bracelets_cat")])
...
async def add_bracelet_desc(msg: types.Message, state: FSMContext):
...
async def pay_yandex(cb: types.CallbackQuery):
    ...
    await cb.message.edit_text(f"✅ Заказ #{order_id} создан!\n\n💰 Сумма: {total}₽\n\n📝 Реквизиты для оплаты:\nЯндекс.Касса: kassa@example.com\n(это демо - укажи свои реквизиты)",
    reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
...
async def pay_crypto(cb: types.CallbackQuery):
    ...
    await cb.message.edit_text(f"✅ Заказ #{order_id} создан!\n\n💰 Сумма: {total}₽\n\n₿ Bitcoin адрес:\nbc1q... (это демо - укажи свой адрес)",
    reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
        [types.InlineKeyboardButton(text="✅ ОПЛАЧЕНО", callback_data=f"confirm_order_{order_id}")],
        [types.InlineKeyboardButton(text="← НАЗАД", callback_data="view_cart")],
    ]))
