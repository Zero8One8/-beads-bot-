...
def init_db():
    ...
    conn.commit()
    c.execute('''CREATE TABLE IF NOT EXISTS knowledge (id INTEGER PRIMARY KEY AUTOINCREMENT, stone_name TEXT, emoji TEXT, properties TEXT, elements TEXT, zodiac TEXT, chakra TEXT, photo_file_id TEXT, created_at TIMESTAMP)''')
...
async def quiz_start(cb: types.CallbackQuery, state: FSMContext):
    ...
    q = QUIZ_QUESTIONS[0]
    kb = types.InlineKeyboardMarkup(inline_keyboard=[
        [types.InlineKeyboardButton(text=o[0], callback_data=f"quiz_ans_{o[1]}")] for o in q["options"]
    ] + [[types.InlineKeyboardButton(text="❌ Отмена", callback_data="menu")]])
    await cb.message.edit_text(q["text"], reply_markup=kb)
    await state.set_state(QuizStates.q1)
    await cb.answer()

async def _next_quiz(cb, state, next_idx, next_state):
    ans = cb.data.replace("quiz_ans_", "")
    data = await state.get_data()
    answers = data.get("quiz_answers", []) + [ans]
    await state.update_data(quiz_answers=answers)
    if next_idx >= len(QUIZ_QUESTIONS):
        stone, emoji, desc, why = get_stone_rec(answers)
        text = f"🔮 ВАШ КАМЕНЬ — {stone} {emoji}\n\n{desc}\n\n✨ {why}\n\nРекомендуем браслет или чётки с {stone.lower()}!\n\nХотите пройти персональную диагностику по фото?"
        conn = get_db(); c = conn.cursor()
        c.execute("INSERT INTO quiz_results (user_id, answers, recommended_stone, created_at) VALUES (?,?,?,?)",
                  (cb.from_user.id, ",".join(answers), stone, datetime.now()))
        conn.commit(); conn.close()
        await cb.message.edit_text(text, reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
            [types.InlineKeyboardButton(text="🩺 ПРОЙТИ ДИАГНОСТИКУ ПО ФОТО", callback_data="diag_start")],
...
@main_router.callback_query(QuizStates.q1, F.data.startswith("quiz_ans_"))
async def quiz_a1(cb: types.CallbackQuery, state: FSMContext):
    await _next_quiz(cb, state, 1, QuizStates.q2)

@main_router.callback_query(QuizStates.q2, F.data.startswith("quiz_ans_"))
async def quiz_a2(cb: types.CallbackQuery, state: FSMContext):
    await _next_quiz(cb, state, 2, QuizStates.q3)

@main_router.callback_query(QuizStates.q3, F.data.startswith("quiz_ans_"))
async def quiz_a3(cb: types.CallbackQuery, state: FSMContext):
    await _next_quiz(cb, state, 3, QuizStates.q4)

@main_router.callback_query(QuizStates.q4, F.data.startswith("quiz_ans_"))
async def quiz_a4(cb: types.CallbackQuery, state: FSMContext):
    await _next_quiz(cb, state, 4, QuizStates.q5)

@main_router.callback_query(QuizStates.q5, F.data.startswith("quiz_ans_"))
async def quiz_a5(cb: types.CallbackQuery, state: FSMContext):
    await _next_quiz(cb, state, 5, QuizStates.q5)
...
async def stone_detail(cb: types.CallbackQuery):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT stone_name, emoji, properties, elements, zodiac, chakra, photo_file_id FROM knowledge WHERE id=?", (stone_id,))
    s = c.fetchone(); conn.close()
    if not s:
        await cb.answer("Не найдено", show_alert=True); return
    name, emoji, props, elements, zodiac, chakra, photo_id = s
    text = (f"{emoji} {name.upper()}\n\n✨ Свойства:\n{props}\n\n"
            f"🌿 Стихии: {elements or '-'}\n♈ Зодиак: {zodiac or '-'}\n🧘 Чакра: {chakra or '-'}")
    kb = types.InlineKeyboardMarkup(inline_keyboard=[
    ...
    try:
        if photo_id:
            await cb.message.answer_photo(photo=photo_id, caption=text, reply_markup=kb)
...
async def ka_set_emoji(msg: types.Message, state: FSMContext):
    await state.update_data(stone_emoji=msg.text)
    await msg.answer("Опишите свойства камня:")
    await state.set_state(KnowledgeAdminStates.stone_properties)

@admin_router.message(KnowledgeAdminStates.stone_properties)
async def ka_set_properties(msg: types.Message, state: FSMContext):
    await state.update_data(stone_properties=msg.text)
    await msg.answer("Стихии (или прочерк -):")
    await state.set_state(KnowledgeAdminStates.stone_elements)

@admin_router.message(KnowledgeAdminStates.stone_elements)
async def ka_set_elements(msg: types.Message, state: FSMContext):
    await state.update_data(stone_elements=msg.text)
    await msg.answer("Знаки зодиака (или прочерк -):")
    await state.set_state(KnowledgeAdminStates.stone_zodiac)

@admin_router.message(KnowledgeAdminStates.stone_zodiac)
async def ka_set_zodiac(msg: types.Message, state: FSMContext):
    await state.update_data(stone_zodiac=msg.text)
    await msg.answer("Чакра (или прочерк -):")
    await state.set_state(KnowledgeAdminStates.stone_chakra)

@admin_router.message(KnowledgeAdminStates.stone_chakra)
async def ka_set_chakra(msg: types.Message, state: FSMContext):
    await state.update_data(stone_chakra=msg.text)
    await msg.answer("Отправьте фото камня или напишите 'пропустить':")
    await state.set_state(KnowledgeAdminStates.stone_photo)
