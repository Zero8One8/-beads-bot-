...
async def knowledge_list_cb(cb: CallbackQuery):
    """База знаний - список всех 31 камня."""
    with db_cursor() as c:
        c.execute("SELECT stone_name, emoji, short_desc FROM knowledge ORDER BY stone_name")
        stones = c.fetchall()
    
    text = "📚 *БАЗА ЗНАНИЙ О КАМНЯХ*\n\n"
    text += f"Выберите камень для подробного описания ({len(stones)} камней):\n\n"
    
    # Создаем кнопки для каждого камня
    kb_buttons = []
    for stone in stones:
        button = InlineKeyboardButton(
            text=f"{stone['emoji']} {stone['stone_name']}",
            callback_data=f"stone_info:{stone['stone_name']}"
        )
        kb_buttons.append([button])
    
    # Добавляем кнопку "назад"
    kb_buttons.append([InlineKeyboardButton(text="← НАЗАД", callback_data="menu")])
    
    kb = InlineKeyboardMarkup(inline_keyboard=kb_buttons)
    await safe_edit(cb, text, parse_mode="Markdown", reply_markup=kb)
    await cb.answer()

@main_router.callback_query(F.data.startswith("stone_info:"))
@rate_limit(2.0)
async def stone_info_cb(cb: CallbackQuery):
    """Показать подробную информацию о камне."""
    stone_name = cb.data.replace("stone_info:", "")
    
    with db_cursor() as c:
        c.execute("""SELECT stone_name, emoji, short_desc, full_desc, color, 
                            chakra, properties, price_per_bead, forms, notes
                     FROM knowledge WHERE stone_name = ?""", (stone_name,))
        stone = c.fetchone()
    ...
    text = f"{stone['emoji']} *{stone['stone_name'].upper()}*\n\n"
    text += f"*Описание:*\n{stone['full_desc']}\n\n"
    text += f"*Цвет:* {stone['color']}\n"
    text += f"*Чакра:* {stone['chakra']}\n"
    text += f"*Свойства:* {stone['properties']}\n"
    text += f"*Цена за бусину:* {stone['price_per_bead']} руб\n"
    text += f"*Доступные размеры:* {stone['forms']}\n"
    text += f"*Особенности:* {stone['notes']}\n"
    
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="← НАЗАД К СПИСКУ", callback_data="knowledge_list")],
