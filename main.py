...
load_dotenv(override=False)  # override=False - не перезаписывает переменные Railway
...
def get_referral_status(referral_count: int) -> str:
    if referral_count >= 16:
        return "👑 Амбассадор"
    elif referral_count >= 6:
        return "⭐ Партнёр"
...
async def notify_admin_order(user_id: int, order_id: int, total: float, method: str):
    try:
        ...
        u = c.fetchone()
        conn.close()
        name = u[0] if u else "Неизвестно"
        username = f"@{u[1]}" if u and u[1] else "нет"
        ...
        logger.error(f"Ошибка уведомления о заказе: {e}")

async def notify_admin_diagnostic(user_id: int, notes: str, photo1_id: str, photo2_id: str):
    try:
        conn = get_db()
        c = conn.cursor()
        c.execute('SELECT first_name, username FROM users WHERE user_id = ?', (user_id,))
        ...
        logger.error(f"Ошибка уведомления о диагностике: {e}")
...
async def admin_panel_keyboard():
    return types.InlineKeyboardMarkup(inline_keyboard=[
        ...
        [types.InlineKeyboardButton(text="📖 ИСТОРИИ КЛИЕНТОВ", callback_data="admin_stories")],
    ...
    if is_admin(user_id):
        ...
        if is_new:
            welcome_text = (
                ...
                "Я помогу вам найти браслет или чётки, которые идеально подойдут именно вам.\n\n"
                "🎁 СПЕЦИАЛЬНО ДЛЯ ВАС — скидка 20% на первый заказ!\n"
                "Промокод: WELCOME20\n\n"
                "Просто выберите нужный раздел 👇"
            )
            await msg.answer(welcome_text, reply_markup=kb)
            # Помечаю что оффер отправлен
            conn = get_db()
            c = conn.cursor()
...
async def contact_master(cb: types.CallbackQuery, state: FSMContext):
    await cb.message.edit_text(
        "📞 СВЯЗЬ С МАСТЕРОМ\n\n"
        "Напишите ваш вопрос — мастер ответит в ближайшее время.\n\n"
        "Или свяжитесь напрямую:",
        reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
            [types.InlineKeyboardButton(text="💬 Написать сообщение боту", callback_data="send_to_master")],
            [types.InlineKeyboardButton(text="← НАЗАД", callback_data="menu")],
        ])
    )
    await cb.answer()
...
async def send_to_master_start(cb: types.CallbackQuery, state: FSMContext):
    await cb.message.edit_text("✍️ Напишите ваш вопрос или сообщение мастеру:")
...
async def send_to_master_finish(msg: types.Message, state: FSMContext):
    ...
    try:
        await bot.send_message(ADMIN_ID, text, reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
            [types.InlineKeyboardButton(text="↩️ Ответить", url=f"tg://user?id={user.id}")],
        ]))
        await msg.answer("✅ Ваше сообщение отправлено мастеру! Ожидайте ответа.")
    except Exception as e:
        await msg.answer("✅ Сообщение отправлено!")
        logger.error(f"Ошибка отправки сообщения мастеру: {e}")
    await state.clear()
...
async def my_referral(cb: types.CallbackQuery):
    ...
    status = get_referral_status(ref_count)
    ...
    text = (
        ...
        f"Ваш % с продаж друзей: {percent}%\n\n"
        f"💰 Баланс бонусов: {balance:.0f} руб\n"
        f"📈 Всего заработано: {total_earned:.0f} руб\n\n"
        f"🔗 Ваша ссылка:\n{ref_link}\n\n"
        f"📊 УРОВНИ:\n"
        f"1-5 друзей → 5% с их покупок\n"
        f"6-15 друзей → 10% с их покупок\n"
        f"16+ друзей → 15% + статус 👑 Амбассадор + скидка 15% лично вам"
    )
    await cb.message.edit_text(text, reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
        [types.InlineKeyboardButton(text="← НАЗАД", callback_data="menu")],
    ]))
...
async def admin_stats(cb: types.CallbackQuery):
    ...
    c.execute('SELECT COALESCE(SUM(total_price), 0) FROM orders WHERE status = ?', ('confirmed',))
    total_revenue = c.fetchone()[0]

    c.execute("SELECT COALESCE(SUM(total_price), 0) FROM orders WHERE status = ? AND DATE(created_at) = DATE('now')", ('confirmed',))
    revenue_today = c.fetchone()[0]

    c.execute('SELECT COUNT(*) FROM diagnostics')
    total_diag = c.fetchone()[0]

    c.execute('SELECT COUNT(*) FROM referrals')
    total_refs = c.fetchone()[0]

    c.execute('SELECT COUNT(*) FROM bracelets')
    total_bracelets = c.fetchone()[0]
    ...
    text = (
        ...
        f"💰 ВЫРУЧКА:\n"
        f"  Всего: {total_revenue:.0f} руб\n"
        f"  Сегодня: {revenue_today:.0f} руб\n\n"
        f"🩺 Диагностик: {total_diag}\n"
        f"🤝 Рефералов: {total_refs}\n"
        f"💎 Браслетов в каталоге: {total_bracelets}"
    )
    await cb.message.edit_text(text, reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
        [types.InlineKeyboardButton(text="← НАЗАД", callback_data="admin_panel")],
    ]))
...
async def admin_broadcast_send(cb: types.CallbackQuery, state: FSMContext):
    ...
    sent = 0
    failed = 0
    await cb.message.edit_text(f"📤 Отправляю рассылку {len(users)} пользователям...")
...
async def show_stories(cb: types.CallbackQuery):
    conn = get_db()
    c = conn.cursor()
    ...
    if not stories:
        await cb.message.edit_text(
            ...
            reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
                [types.InlineKeyboardButton(text="✍️ Поделиться своей историей", callback_data="add_story")],
                [types.InlineKeyboardButton(text="← НАЗАД", callback_data="menu")],
            ])
        )
    else:
        for story in stories:
            ...
            try:
                if story[1]:
                    await cb.message.answer_photo(photo=story[1], caption=text)
                else:
                    await cb.message.answer(text)
            except:
                await cb.message.answer(text)
        await cb.message.answer(
            "📖 Это истории наших клиентов\n\nПоделитесь и вы!",
            reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
                [types.InlineKeyboardButton(text="✍️ Поделиться своей историей", callback_data="add_story")],
                [types.InlineKeyboardButton(text="← НАЗАД", callback_data="menu")],
            ])
...
async def add_story_start(cb: types.CallbackQuery, state: FSMContext):
    await cb.message.edit_text(
        "✍️ ПОДЕЛИТЕСЬ СВОЕЙ ИСТОРИЕЙ\n\nРасскажите как наши браслеты или услуги помогли вам.\nНапишите текст:"
    )
...
async def add_story_text(msg: types.Message, state: FSMContext):
    ...
    await msg.answer(
        "📸 Прикрепите фото (необязательно) или нажмите кнопку ниже:",
        reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
...
async def add_story_photo(msg: types.Message, state: FSMContext):
    ...
    try:
        ...
        pass
    await msg.answer("✅ Спасибо! История отправлена на проверку и скоро появится в боте.")
    await state.clear()
...
async def story_no_photo(cb: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        pass
    await cb.message.edit_text("✅ Спасибо! История отправлена на проверку.")
    await state.clear()
    await cb.answer()
