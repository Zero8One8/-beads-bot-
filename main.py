...
async def manage_subcat(cb: types.CallbackQuery, state: FSMContext):
    ...
    if subcats:
        for subcat in subcats:
            text += f"{subcat[2]} {subcat[1]}\n"
            buttons.append([types.InlineKeyboardButton(text=f"✏️ {subcat[1]}", callback_data=f"edit_subcat_{subcat[0]}")])
    else:
...
async def manage_subsubcat(cb: types.CallbackQuery, state: FSMContext):
    ...
    if subsubcats:
        for subsubcat in subsubcats:
            ...
            buttons.append([types.InlineKeyboardButton(text=f"✏️ {subsubcat[1]}", callback_data=f"edit_subsubcat_{subsubcat[0]}")])
    else:
