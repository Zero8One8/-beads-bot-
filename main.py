...
def is_spam(user_id: int, cooldown: float = 1.0) -> bool:
    now = time.monotonic()
    last = _last_cb.get(user_id, 0)
...
def _get_database_url() -> str:
    """Читает DATABASE_URL каждый раз заново — надёжнее при Railway деплое."""
    url = os.getenv('DATABASE_URL', '')
    if not url:
        # Пробуем альтернативные имена которые Railway иногда использует
        url = os.getenv('POSTGRES_URL', '') or os.getenv('POSTGRESQL_URL', '')
...
def get_db():
    ...
    if not url:
        ...
        env_keys = [k for k in os.environ.keys() if 'postgres' in k.lower() or 'database' in k.lower() or 'db' in k.lower()]
        logger.error(f"DATABASE_URL не найден. Переменные окружения с 'db/postgres': {env_keys}")
        raise RuntimeError("❌ DATABASE_URL не установлен! Добавьте переменную в Railway.")
    try:
...
async def set_setting(key: str, value: str):
    conn = get_db(); c = conn.cursor()
    c.execute('INSERT INTO bot_settings (key, value) VALUES (%s, %s) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value', (key, value))
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS categories 
                 (id SERIAL PRIMARY KEY, name TEXT UNIQUE, emoji TEXT, desc TEXT)''')
    
    # Контент (текст в категориях)
    c.execute('''CREATE TABLE IF NOT EXISTS content 
                 (id SERIAL PRIMARY KEY, cat_id INT, title TEXT, desc TEXT, created_at TIMESTAMP)''')
    
    # Тренировки
    c.execute('''CREATE TABLE IF NOT EXISTS workouts 
                 (id SERIAL PRIMARY KEY, name TEXT, desc TEXT, duration INT, difficulty TEXT, created_at TIMESTAMP)''')
    
    # Музыка
    c.execute('''CREATE TABLE IF NOT EXISTS music 
                 (id SERIAL PRIMARY KEY, name TEXT, desc TEXT, duration INT, audio_url TEXT, created_at TIMESTAMP)''')
    
    # Услуги
    c.execute('''CREATE TABLE IF NOT EXISTS services 
                 (id SERIAL PRIMARY KEY, name TEXT, desc TEXT, price REAL, created_at TIMESTAMP)''')
    
    # Диагностика (фото)
    c.execute('''CREATE TABLE IF NOT EXISTS diagnostics 
                 (id SERIAL PRIMARY KEY, user_id INT, photo_count INT, notes TEXT, created_at TIMESTAMP, admin_result TEXT, sent BOOLEAN DEFAULT FALSE, photo1_file_id TEXT, photo2_file_id TEXT, followup_sent INT DEFAULT 0)''')
    
    # Браслеты
    c.execute('''CREATE TABLE IF NOT EXISTS bracelets 
                 (id SERIAL PRIMARY KEY, name TEXT, desc TEXT, price REAL, image_url TEXT, created_at TIMESTAMP)''')
    
    # Корзина
    c.execute('''CREATE TABLE IF NOT EXISTS cart 
                 (id SERIAL PRIMARY KEY, user_id INT, bracelet_id INT, quantity INT, added_at TIMESTAMP)''')
    
    # Заказы
    c.execute('''CREATE TABLE IF NOT EXISTS orders 
                 (id SERIAL PRIMARY KEY, user_id INT, total_price REAL, status TEXT, payment_method TEXT, created_at TIMESTAMP)''')
    
    # Отзывы
    c.execute('''CREATE TABLE IF NOT EXISTS reviews 
                 (id SERIAL PRIMARY KEY, user_id INT, bracelet_id INT, rating INT, text TEXT, created_at TIMESTAMP)''')
    
    # Подкатегории
    c.execute('''CREATE TABLE IF NOT EXISTS subcategories 
                 (id SERIAL PRIMARY KEY, parent_id INT, name TEXT, emoji TEXT, created_at TIMESTAMP)''')
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS referrals (id SERIAL PRIMARY KEY, referrer_id INT, referred_id INT, created_at TIMESTAMP)''')
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS stories (id SERIAL PRIMARY KEY, user_id INT, text TEXT, photo_file_id TEXT, approved BOOLEAN DEFAULT FALSE, created_at TIMESTAMP)''')
    c.execute('''CREATE TABLE IF NOT EXISTS broadcasts (id SERIAL PRIMARY KEY, text TEXT, created_at TIMESTAMP, sent_count INT DEFAULT 0)''')
    conn.commit()
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS showcase_items
                 (id SERIAL PRIMARY KEY, collection_id INT, name TEXT, desc TEXT,
                  price REAL, image_file_id TEXT, sort_order INT DEFAULT 0, created_at TIMESTAMP)''')
    c.execute('''CREATE TABLE IF NOT EXISTS knowledge (id SERIAL PRIMARY KEY, stone_name TEXT UNIQUE, emoji TEXT, properties TEXT, elements TEXT, zodiac TEXT, chakra TEXT, photo_file_id TEXT, created_at TIMESTAMP, short_desc TEXT, full_desc TEXT, color TEXT, stone_id TEXT, tasks TEXT, price_per_bead INTEGER, forms TEXT, notes TEXT)''')
    c.execute('''CREATE TABLE IF NOT EXISTS quiz_results (id SERIAL PRIMARY KEY, user_id INT, answers TEXT, recommended_stone TEXT, created_at TIMESTAMP)''')
    conn.commit()
    ...
    for _sql in ["ALTER TABLE users ADD COLUMN welcome_sent BOOLEAN DEFAULT FALSE","ALTER TABLE users ADD COLUMN referred_by INT DEFAULT NULL","ALTER TABLE diagnostics ADD COLUMN followup_sent INT DEFAULT 0","ALTER TABLE knowledge ADD COLUMN short_desc TEXT","ALTER TABLE knowledge ADD COLUMN full_desc TEXT","ALTER TABLE knowledge ADD COLUMN color TEXT","ALTER TABLE knowledge ADD COLUMN stone_id TEXT","ALTER TABLE knowledge ADD COLUMN tasks TEXT","ALTER TABLE knowledge ADD COLUMN price_per_bead INTEGER","ALTER TABLE knowledge ADD COLUMN forms TEXT","ALTER TABLE knowledge ADD COLUMN notes TEXT"]:
        try: c.execute(_sql.replace("ADD COLUMN", "ADD COLUMN IF NOT EXISTS")); conn.commit()
        except Exception as _e: 
            conn.rollback()
            if "already exists" not in str(_e): logger.debug(f"ALTER: {_e}")
    ...
    try:
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Розовый кварц', '💎', 'love, healing, self_love', '💚', 'Камень безусловной любви', 'Розовый кварц - камень сердца. Мягко раскрывает сердце, исцеляет старые раны, помогает в отношениях с собой и другими. Самый мощный камень для работы с любовью и прощением.', 'розовый', 'rose_quartz', 'love, healing, self_love', 50, '6mm, 8mm, 10mm, 12mm', 'Универсален, безопасен, один из самых популярных'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Цитрин', '💎', 'money, confidence, joy', '🟡, 🟠', 'Камень денег и радости', 'Цитрин привлекает достаток и процветание. Усиливает личную силу, помогает верить в себя. Один из самых мощных камней для денежной энергии.', 'жёлтый', 'citrine', 'money, confidence, joy', 80, '6mm, 8mm, 10mm, 12mm', 'Один из лучших для денег, редкий натуральный'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Аметист', '💎', 'meditation, clarity, sobriety', '⚪, 💜', 'Камень медитаций и трезвости', 'Аметист - духовный камень. Поддерживает медитации, защищает от зависимостей, успокаивает ум. Классический камень для практик и внутренней работы.', 'фиолетовый', 'amethyst', 'meditation, clarity, sobriety', 60, '6mm, 8mm, 10mm, 12mm', 'Универсален, подходит всем знакам зодиака'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Лабрадорит', '💎', 'transformation, intuition, magic', '💜, ⚪', 'Камень трансформации', 'Лабрадорит раскрывает скрытые способности, помогает видеть то, что за пределами видимого. Камень магии и преобразования. Один из самых мощных для духовного развития.', 'серый с переливом', 'labradorite', 'transformation, intuition, magic', 100, '6mm, 8mm, 10mm', 'Драгоценный камень, требует бережного обращения'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Чёрный турмалин', '💎', 'protection, grounding, boundaries', '🔴', 'Камень защиты', 'Чёрный турмалин - сильнейший защитник. Создаёт энергетический щит, защищает от чужого влияния, заземляет энергию. Незаменим для чувствительных людей.', 'чёрный', 'black_tourmaline', 'protection, grounding, boundaries', 120, '6mm, 8mm, 10mm', 'Самый мощный защитник, работает на уровне корневой чакры'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Зелёный авантюрин', '💎', 'luck, prosperity, opportunity', '💚', 'Камень удачи', 'Зелёный авантюрин привлекает удачу и новые возможности. Помогает видеть пути вперёд, раскрывает двери. Камень процветания на материальном уровне.', 'зелёный', 'green_aventurine', 'luck, prosperity, opportunity', 40, '6mm, 8mm, 10mm, 12mm', 'Доступен, работает быстро, хороший для начинающих'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Лунный камень', '💎', 'intuition, feminine_energy, inner_light', '🟠, ⚪', 'Камень женской энергии', 'Лунный камень связан с луной и интуицией. Раскрывает внутреннюю мудрость, поддерживает женские энергии, помогает доверять интуиции.', 'молочный с сиянием', 'moonstone', 'intuition, feminine_energy, inner_light', 90, '6mm, 8mm, 10mm', 'Мощен для женщин, усиливает интуицию'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Тигровый глаз', '💎', 'courage, action, willpower', '🟡', 'Камень мужества', 'Тигровый глаз даёт силу и мужество. Помогает действовать смело, преодолевать страхи. Камень для воинов и лидеров.', 'коричневый с полосами', 'tiger_eye', 'courage, action, willpower', 150, '6mm, 8mm, 10mm, 12mm', 'Драгоценный, очень популярен, долговечен'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Горный хрусталь', '💎', 'amplification, clarity, programming', '🌈', 'Универсальный усилитель', 'Горный хрусталь - усилитель всех энергий. Можно программировать на любое намерение. Один из самых универсальных и мощных камней.', 'прозрачный', 'clear_quartz', 'amplification, clarity, programming', 35, '6mm, 8mm, 10mm, 12mm', 'Лучше всего использовать в центре браслета'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Гематит', '💎', 'grounding, protection, stability', '🔴', 'Камень заземления', 'Гематит заземляет, возвращает в реальность, защищает. Идеален для людей, которые часто витают в облаках. Стабилизирует энергию.', 'чёрный металлический', 'hematite', 'grounding, protection, stability', 70, '6mm, 8mm, 10mm, 12mm', 'Тяжелый, создаёт ощущение защиты'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Родонит', '💎', 'healing, trauma, self_care', '💚', 'Камень исцеления травм', 'Родонит помогает исцелить эмоциональные раны. Работает с давними болями и обидами. Нежный, но мощный камень для глубокой работы.', 'розовый с чёрными прожилками', 'rhodonite', 'healing, trauma, self_care', 85, '6mm, 8mm, 10mm', 'Отличен для глубокого исцеления'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Содалит', '💎', 'clarity, expression, truth', '💜, 🔵', 'Камень ясности и правды', 'Содалит развивает интуицию, помогает выразить правду. Успокаивает ум, улучшает ясность мышления. Камень для честного общения.', 'синий с белыми прожилками', 'sodalite', 'clarity, expression, truth', 65, '6mm, 8mm, 10mm', 'Помогает в общении, развивает интуицию'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Сердолик', '💎', 'creativity, passion, vitality', '🟠, 🟡', 'Камень творчества и страсти', 'Сердолик пробуждает творчество и жизненную энергию. Помогает воплощать идеи, даёт мотивацию. Камень для творческих людей.', 'оранжево-красный', 'carnelian', 'creativity, passion, vitality', 75, '6mm, 8mm, 10mm', 'Натуральный сердолик редкий, работает с подсознанием'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Лепидолит', '💎', 'calm, anxiety, transition', '💚, ⚪', 'Камень спокойствия', 'Лепидолит содержит литий - природное успокаивающее. Помогает при тревоге, поддерживает в переходные периоды. Мягкий и нежный камень.', 'фиолетовый-розовый', 'lepidolite', 'calm, anxiety, transition', 95, '6mm, 8mm', 'Натуральный литий, помогает при стрессе'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Флюорит', '💎', 'focus, organization, mental_clarity', '💜', 'Камень ясности ума', 'Флюорит улучшает концентрацию, организует мысли, помогает в обучении. Камень для студентов и интеллектуальной работы.', 'фиолетовый, зелёный, жёлтый', 'fluorite', 'focus, organization, mental_clarity', 110, '6mm, 8mm, 10mm', 'Хрупкий, требует бережного обращения, очень мощен для ума'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Синий авантюрин', '💎', 'communication, inner_peace, harmony', '🔵, 💜', 'Камень спокойного общения', 'Синий авантюрин помогает спокойно выражать себя, создаёт внутреннюю гармонию. Успокаивающий камень для нервной системы.', 'синий', 'aventurine_blue', 'communication, inner_peace, harmony', 55, '6mm, 8mm, 10mm', 'Редкий вид авантюрина, очень мягкий'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Обсидиан', '💎', 'protection, truth, grounding', '🔴', 'Камень правды', 'Обсидиан защищает от иллюзий, помогает видеть правду. Мощный защитник, но требует уважения. Камень для глубокой работы с тенью.', 'чёрный глянцевый', 'obsidian', 'protection, truth, grounding', 130, '6mm, 8mm, 10mm', 'Вулканический камень, очень мощен, требует опыта'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Нефрит', '💎', 'harmony, longevity, protection', '💚', 'Камень гармонии', 'Нефрит в восточной традиции - камень долголетия и гармонии. Защищает, приносит равновесие. Камень мудрости и стабильности.', 'зелёный', 'jade', 'harmony, longevity, protection', 140, '6mm, 8mm, 10mm', 'Драгоценный, ценится в восточной культуре'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Спектролит', '💎', 'magic, intuition, mystery', '💜, ⚪', 'Камень магии', 'Редкий вид лабрадорита с ярким переливом. Один из самых магических камней. Открывает двери в невидимые миры.', 'чёрный с радужным переливом', 'labradorite_spectrolite', 'magic, intuition, mystery', 200, '8mm, 10mm', 'Очень редкий и мощный, для опытных'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Кунцит', '💎', 'unconditional_love, peace, spiritual_love', '💚', 'Камень безусловной любви', 'Кунцит - камень духовной любви. Раскрывает сердце на глубоком уровне. Камень миролюбия и сострадания ко всему живому.', 'розово-фиолетовый', 'kunzite', 'unconditional_love, peace, spiritual_love', 180, '8mm, 10mm', 'Редкий, хрупкий, очень нежный и мощный'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Малахит', '💎', 'transformation, protection, prosperity', '💚, 🟡', 'Камень трансформации', 'Малахит - мощный трансформер. Защищает путешественников, помогает в больших переменах. Очень энергичный камень.', 'зелёный с чёрными полосами', 'malachite', 'transformation, protection, prosperity', 160, '10mm, 12mm', 'Ядовит в пыли, не лизать, работать осторожно'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Амазонит', '💎', 'truth, communication, boundaries', '🔵, 💚', 'Камень правдивого слова', 'Амазонит помогает говорить правду с добротой. Поддерживает здоровые границы в общении. Камень женщины-воина.', 'голубовато-зелёный', 'amazonite', 'truth, communication, boundaries', 70, '6mm, 8mm, 10mm', 'Помогает в конфликтах, работает с горлом'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Розовый турмалин (Турмалин)', '💎', 'divine_love, compassion, healing', '💚', 'Камень божественной любви', 'Розовый турмалин раскрывает божественную любовь в сердце. Очень нежный и мощный. Камень для глубокого исцеления сердца.', 'розовый', 'tourmaline_pink', 'divine_love, compassion, healing', 190, '8mm, 10mm', 'Редкий и дорогой, для преданных любви'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Шерл (чёрный турмалин сырой)', '💎', 'deep_protection, detox, grounding', '🔴', 'Мощная защита', 'Сырой чёрный турмалин - наиболее мощный вариант. Детоксирует энергию, глубоко защищает. Для опытных работников.', 'чёрный матовый', 'tourmaline_black_schorl', 'deep_protection, detox, grounding', 170, '10mm', 'Сырой, очень мощный, требует уважения'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Натуральный цитрин', '💎', 'money, abundance, joy', '🟡', 'Редкий натуральный цитрин', 'Редкий натуральный цитрин (не нагревается). Один из самых мощных для денег и радости. Ценный камень для истинной работы.', 'жёлтый натуральный', 'citrine_natural', 'money, abundance, joy', 220, '8mm, 10mm', 'Редкий и дорогой, настоящий цитрин'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Многоцветный турмалин', '💎', 'harmony, balance, wholeness', '🌈', 'Камень гармонии всех энергий', 'Редкий турмалин с несколькими цветами в одном кристалле. Гармонизирует все чакры сразу. Камень целостности и интеграции.', 'разноцветный', 'tourmaline_multicolor', 'harmony, balance, wholeness', 250, '10mm', 'Очень редкий, для продвинутых практиков'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Гранат', '💎', 'vitality, passion, grounding', '🔴, 🟠', 'Камень жизненной силы', 'Гранат пробуждает сексуальность и жизненную энергию. Камень страсти и земной силы. Помогает заземляться в тело.', 'красный-коричневый', 'garnet', 'vitality, passion, grounding', 145, '6mm, 8mm, 10mm', 'Драгоценный, помогает с либидо и энергией'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Ляпис-лазурь', '💎', 'wisdom, truth, inner_sight', '💜, 🔵', 'Камень небесной мудрости', 'Ляпис-лазурь - камень королей и мудрецов. Открывает третий глаз, связывает с высшей мудростью. Один из самых ценных камней.', 'глубокий синий с золотом', 'lapis_lazuli', 'wisdom, truth, inner_sight', 210, '8mm, 10mm', 'Очень дорогой, содержит золотой пирит'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Апатит синий', '💎', 'psychic_ability, clarity, communication', '🔵, 💜', 'Камень психических способностей', 'Синий апатит развивает психические способности, ясновидение, яснослышание. Помогает в медитации и внутреннем видении.', 'синий', 'apatite_blue', 'psychic_ability, clarity, communication', 125, '8mm, 10mm', 'Редкий, мощен для психического развития'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Кунцит сиреневый', '💎', 'divine_love, angels, spirituality', '💚, ⚪', 'Камень ангельской любви', 'Редкий сиреневый кунцит помогает связаться с ангельским царством. Очень высокая вибрация. Камень для духовных практик.', 'сиреневый-фиолетовый', 'kunzite_lilac', 'divine_love, angels, spirituality', 240, '8mm, 10mm', 'Хрупкий, редкий, для опытных практиков'))
        c.execute("INSERT INTO knowledge (stone_name, emoji, properties, chakra, short_desc, full_desc, color, stone_id, tasks, price_per_bead, forms, notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (stone_name) DO NOTHING", ('Арбузный турмалин', '💎', 'love_balance, yin_yang, integration', '💚', 'Камень баланса любви', 'Редкий турмалин с розовым центром и зелёной оболочкой - символ инь и ян. Баланс мужского и женского. Редкий и мощный камень.', 'розовый центр, зелёная оболочка', 'watermelon_tourmaline', 'love_balance, yin_yang, integration', 260, '10mm', 'Очень редкий, символ целостности'))
        conn.commit()
    except Exception as _e:
    ...
    try:
        c.execute("INSERT INTO categories (name, emoji, desc) VALUES (%s, %s, %s) ON CONFLICT (name) DO NOTHING", ('🏋️ Практики', '🏋️', 'Физические упражнения'))
        c.execute("INSERT INTO categories (name, emoji, desc) VALUES (%s, %s, %s) ON CONFLICT (name) DO NOTHING", ('🎵 Музыка 432Hz', '🎵', 'Исцеляющая музыка'))
        c.execute("INSERT INTO categories (name, emoji, desc) VALUES (%s, %s, %s) ON CONFLICT (name) DO NOTHING", ('🎁 Готовые браслеты', '🎁', 'Готовые изделия'))
        c.execute("INSERT INTO categories (name, emoji, desc) VALUES (%s, %s, %s) ON CONFLICT (name) DO NOTHING", ('✨ Индивидуальный подбор', '✨', 'Подбор под вас'))
        c.execute("INSERT INTO categories (name, emoji, desc) VALUES (%s, %s, %s) ON CONFLICT (name) DO NOTHING", ('💍 Браслеты на заказ', '💍', 'Индивидуальный заказ браслета'))
        conn.commit()
    except:
        pass
    
    try:
        c.execute("INSERT INTO admins VALUES (%s) ON CONFLICT DO NOTHING", (ADMIN_ID,))
        conn.commit()
    except:
        pass
...
def is_admin(user_id):
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute('SELECT admin_id FROM admins WHERE admin_id = %s', (user_id,))
    result = c.fetchone()
    conn.close()
...
async def notify_admin_order(user_id, order_id, total, method):
    try:
        conn = get_db(); c = conn.cursor()
        c.execute('SELECT first_name, username FROM users WHERE user_id = %s', (user_id,))
...
async def admin_panel_keyboard():
    return types.InlineKeyboardMarkup(inline_keyboard=[
        ...
        [types.InlineKeyboardButton(text="💎 ВИТРИНА (КОЛЛЕКЦИИ)", callback_data="admin_showcase")],
        [types.InlineKeyboardButton(text="🩺 ДИАГНОСТИКА / ЗАКАЗЫ", callback_data="admin_diagnostics")],
...
async def start(msg: types.Message, state: FSMContext):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute('SELECT user_id, welcome_sent FROM users WHERE user_id = %s', (user_id,))
    existing = c.fetchone()
    ...
    if is_new:
        c.execute('INSERT INTO users (user_id, username, first_name, created_at, welcome_sent, referred_by) VALUES (%s, %s, %s, %s, %s, %s) ON CONFLICT (user_id) DO NOTHING',
        ...
        if ref_id:
            ...
            if c.fetchone():
                c.execute('INSERT INTO referrals (referrer_id, referred_id, created_at) VALUES (%s, %s, %s) ON CONFLICT DO NOTHING', (ref_id, user_id, datetime.now()))
                c.execute('INSERT INTO referral_balance (user_id, referral_count) VALUES (%s, 1) ON CONFLICT (user_id) DO UPDATE SET referral_count = referral_balance.referral_count + 1', (ref_id,))
                conn.commit()
                try: await bot.send_message(ref_id, "🎉 По вашей реферальной ссылке зарегистрировался новый пользователь!")
                except: pass
    else:
        c.execute('INSERT INTO users (user_id, username, first_name, created_at) VALUES (%s, %s, %s, %s) ON CONFLICT (user_id) DO NOTHING',
    ...
    if is_admin(user_id):
        ...
        if is_new:
            ...
            conn = get_db(); c = conn.cursor()
            c.execute('UPDATE users SET welcome_sent = TRUE WHERE user_id = %s', (user_id,))
            conn.commit(); conn.close()
        else:
...
async def show_category(cb: types.CallbackQuery, state: FSMContext):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute('SELECT name, emoji FROM categories WHERE id = %s', (cat_id,))
    cat = c.fetchone()
    ...
    c.execute('SELECT id, name, emoji FROM subcategories WHERE parent_id = %s', (cat_id,))
    subcats = c.fetchall()
    
    # ПОТОМ проверяю контент
    c.execute('SELECT title, desc FROM content WHERE cat_id = %s', (cat_id,))
    content = c.fetchall()
    conn.close()
...
async def cat_admin_menu(target, cat_id: int = None):
    ...
    c.execute("SELECT id, name, emoji FROM subcategories WHERE parent_id=%s ORDER BY id", (cat_id,))
    subcats = c.fetchall()
    c.execute("SELECT id, title FROM content WHERE cat_id=%s ORDER BY id", (cat_id,))
    contents = c.fetchall()
...
async def subcat_admin_menu(target, sub_id: int, parent_cat_id: int):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT name, emoji FROM subcategories WHERE id=%s", (sub_id,))
    sub = c.fetchone()
    if not sub: conn.close(); return
    name, emoji = sub
    em = emoji or "📁"

    c.execute("SELECT id, name, emoji FROM subsubcategories WHERE parent_id=%s ORDER BY id", (sub_id,))
    subsubs = c.fetchall()
    c.execute("SELECT id, title FROM content WHERE cat_id=%s ORDER BY id", (sub_id,))
    contents = c.fetchall()
    conn.close()
...
async def ssub_admin_menu(target, ssub_id: int, sub_id: int, cat_id: int):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT name, emoji FROM subsubcategories WHERE id=%s", (ssub_id,))
    ssub = c.fetchone()
    if not ssub: conn.close(); return
    name, emoji = ssub
    c.execute("SELECT id, title FROM content WHERE cat_id=%s ORDER BY id", (ssub_id,))
...
async def cadm_del(cb: types.CallbackQuery):
    ...
    c.execute("SELECT id FROM subcategories WHERE parent_id=%s", (cat_id,))
    subs = c.fetchall()
    for (sid,) in subs:
        c.execute("DELETE FROM subsubcategories WHERE parent_id=%s", (sid,))
        c.execute("DELETE FROM content WHERE cat_id=%s", (sid,))
    c.execute("DELETE FROM subcategories WHERE parent_id=%s", (cat_id,))
    c.execute("DELETE FROM content WHERE cat_id=%s", (cat_id,))
...
async def cadm_new(cb: types.CallbackQuery, state: FSMContext):
    if not is_admin(cb.from_user.id): await cb.answer("❌"); return
    await cb.message.edit_text("📁 Новая категория\n\nВведи название:")
...
async def cadm_save_new(msg: types.Message, state: FSMContext):
    ...
    conn = get_db(); c = conn.cursor()
    try:
        c.execute("INSERT INTO categories (name, emoji, desc) VALUES (%s,%s,%s)", (name, "📁", ""))
        conn.commit()
...
async def cadm_rename(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text("✏️ Введи новое название категории:")
...
async def cadm_save_rename(msg: types.Message, state: FSMContext):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("UPDATE categories SET name=%s WHERE id=%s", (msg.text.strip(), cat_id))
    conn.commit(); conn.close()
...
async def cadm_addsub(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text("📁 Новая подпапка\n\nВведи название:")
...
async def cadm_save_subcat(msg: types.Message, state: FSMContext):
    ...
    conn = get_db(); c = conn.cursor()
    try:
        c.execute("INSERT INTO subcategories (parent_id, name, emoji, created_at) VALUES (%s,%s,%s,%s)",
...
async def cadm_sub(cb: types.CallbackQuery):
    ...
    if len(parts) > 3:
        ...
        conn = get_db(); c = conn.cursor()
        c.execute("SELECT parent_id FROM subcategories WHERE id=%s", (sub_id,))
        r = c.fetchone(); conn.close()
...
async def cadm_delsub(cb: types.CallbackQuery):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT name FROM subcategories WHERE id=%s", (sub_id,))
    sub = c.fetchone()
    c.execute("DELETE FROM subsubcategories WHERE parent_id=%s", (sub_id,))
    c.execute("DELETE FROM content WHERE cat_id=%s", (sub_id,))
...
async def cadm_renamesub(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text("✏️ Введи новое название подпапки:")
...
async def cadm_save_renamesub(msg: types.Message, state: FSMContext):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("UPDATE subcategories SET name=%s WHERE id=%s", (msg.text.strip(), sub_id))
    conn.commit(); conn.close()
...
async def cadm_addssub(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text("📄 Новая под-подпапка\n\nВведи название:")
...
async def cadm_save_ssub(msg: types.Message, state: FSMContext):
    ...
    conn = get_db(); c = conn.cursor()
    try:
        c.execute("INSERT INTO subsubcategories (parent_id, name, emoji, created_at) VALUES (%s,%s,%s,%s)",
...
async def cadm_ssub(cb: types.CallbackQuery):
    ...
    ssub_id = int(parts[2])
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT parent_id FROM subsubcategories WHERE id=%s", (ssub_id,))
    r = c.fetchone()
...
async def cadm_delssub(cb: types.CallbackQuery):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT name FROM subsubcategories WHERE id=%s", (ssub_id,))
    ss = c.fetchone()
    c.execute("DELETE FROM content WHERE cat_id=%s", (ssub_id,))
    c.execute("DELETE FROM subsubcategories WHERE id=%s", (ssub_id,))
    conn.commit(); conn.close()
...
async def cadm_renamessub(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text("✏️ Введи новое название:")
...
async def cadm_save_renamessub(msg: types.Message, state: FSMContext):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("UPDATE subsubcategories SET name=%s WHERE id=%s", (msg.text.strip(), ssub_id))
    conn.commit(); conn.close()
    await msg.answer("✅ Переименовано!")
    await state.clear()
...
async def cadm_addcontent(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text("📝 Добавить контент\n\nВведи заголовок:")
...
async def cadm_content_desc(msg: types.Message, state: FSMContext):
    ...
    conn = get_db(); c = conn.cursor()
    try:
        c.execute("INSERT INTO content (cat_id, title, desc, created_at) VALUES (%s,%s,%s,%s)",
...
async def cadm_delcontent(cb: types.CallbackQuery):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("DELETE FROM content WHERE id=%s", (content_id,))
    # Определяем куда вернуться
    in_cat = c.execute("SELECT id FROM categories WHERE id=%s", (back_id,)).fetchone()
    if not in_cat:
        r = c.execute("SELECT parent_id FROM subcategories WHERE id=%s", (back_id,)).fetchone()
...
async def cadm_view_content(cb: types.CallbackQuery):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT title, desc, cat_id FROM content WHERE id=%s", (content_id,))
...
async def cadm_addtrack(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text("🎵 Новый трек\n\nВведи название трека:")
...
async def cadm_music_file(msg: types.Message, state: FSMContext):
    ...
    conn = get_db(); c = conn.cursor()
    try:
        c.execute("INSERT INTO music (name, desc, duration, audio_url, created_at) VALUES (%s,%s,%s,%s,%s)",
...
async def cadm_deltrack(cb: types.CallbackQuery):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("DELETE FROM music WHERE id=%s", (track_id,))
    conn.commit(); conn.close()
...
async def cadm_view_track(cb: types.CallbackQuery):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT name, duration FROM music WHERE id=%s", (track_id,))
    t = c.fetchone(); conn.close()
    if not t: await cb.answer("Не найдено", show_alert=True); return
    name, dur = t
    conn2 = get_db(); c2 = conn2.cursor()
    c2.execute("SELECT id FROM categories WHERE name LIKE '%🎵%' OR name LIKE '%Музыка%'")
...
async def add_service_process(msg: types.Message, state: FSMContext):
    ...
    c = conn.cursor()
    try:
        c.execute("INSERT INTO services (name, desc, price, created_at) VALUES (%s, %s, %s, %s)",
...
async def edit_categories(cb: types.CallbackQuery):
    ...
    buttons.append([types.InlineKeyboardButton(text="➕ ДОБАВИТЬ", callback_data="add_cat")])
...
async def update_cat_name(msg: types.Message, state: FSMContext):
    ...
    if 'edit_cat_id' in data:
        ...
        conn = get_db()
        c = conn.cursor()
        c.execute('UPDATE categories SET name = %s WHERE id = %s', (msg.text, cat_id))
        conn.commit()
        conn.close()
...
async def handle_add_name(msg: types.Message, state: FSMContext):
    ...
    if 'add_subsubcat_parent_id' in data and data.get('step') == 'name':
        ...
        conn = get_db()
        c = conn.cursor()
        c.execute('UPDATE subsubcategories SET name = %s WHERE id = %s', (msg.text, subsubcat_id))
        conn.commit()
        conn.close()
        ...
        conn = get_db()
        c = conn.cursor()
        c.execute('UPDATE subcategories SET name = %s WHERE id = %s', (msg.text, subcat_id))
        conn.commit()
        conn.close()
...
async def edit_subcat(cb: types.CallbackQuery, state: FSMContext):
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute('SELECT name FROM subcategories WHERE id = %s', (subcat_id,))
    subcat = c.fetchone()
    conn.close()
...
async def update_subcat_name(msg: types.Message, state: FSMContext):
    ...
    try:
        ...
        c.execute('SELECT parent_id FROM subcategories WHERE id = %s', (subcat_id,))
        result = c.fetchone()
        ...
        # Обновляю название
        c.execute('UPDATE subcategories SET name = %s WHERE id = %s', (new_name, subcat_id))
        conn.commit()
        conn.close()
        ...
        conn = get_db()
        c = conn.cursor()
        c.execute('SELECT name FROM categories WHERE id = %s', (parent_id,))
        cat = c.fetchone()
        c.execute('SELECT id, name, emoji FROM subcategories WHERE parent_id = %s', (parent_id,))
        subcats = c.fetchall()
        conn.close()
...
async def delete_subcat(cb: types.CallbackQuery):
    ...
    try:
        ...
        c.execute('SELECT name, parent_id FROM subcategories WHERE id = %s', (subcat_id,))
        subcat = c.fetchone()
        ...
        c.execute('DELETE FROM subsubcategories WHERE parent_id = %s', (subcat_id,))
        
        # Удаляю саму подкатегорию
        c.execute('DELETE FROM subcategories WHERE id = %s', (subcat_id,))
        conn.commit()
        conn.close()
...
async def manage_subsubcat(cb: types.CallbackQuery, state: FSMContext):
    ...
    c.execute('SELECT name FROM subcategories WHERE id = %s', (subcat_id,))
    subcat = c.fetchone()
    c.execute('SELECT id, name, emoji FROM subsubcategories WHERE parent_id = %s', (subcat_id,))
...
async def edit_subsubcat(cb: types.CallbackQuery, state: FSMContext):
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute('SELECT name FROM subsubcategories WHERE id = %s', (subsubcat_id,))
...
async def update_subsubcat_name(msg: types.Message, state: FSMContext):
    ...
    try:
        ...
        # Получаю parent_id для возврата в меню
        c.execute('SELECT parent_id FROM subsubcategories WHERE id = %s', (subsubcat_id,))
        result = c.fetchone()
        ...
        # Обновляю название
        c.execute('UPDATE subsubcategories SET name = %s WHERE id = %s', (new_name, subsubcat_id))
        conn.commit()
        conn.close()
        ...
        conn = get_db()
        c = conn.cursor()
        c.execute('SELECT name FROM subcategories WHERE id = %s', (parent_id,))
        subcat = c.fetchone()
        c.execute('SELECT id, name, emoji FROM subsubcategories WHERE parent_id = %s', (parent_id,))
        subsubcats = c.fetchall()
        conn.close()
...
async def delete_subsubcat(cb: types.CallbackQuery):
    ...
    try:
        ...
        # Получаю информацию перед удалением
        c.execute('SELECT name, parent_id FROM subsubcategories WHERE id = %s', (subsubcat_id,))
        subsubcat = c.fetchone()
        ...
        c.execute('DELETE FROM subsubcategories WHERE id = %s', (subsubcat_id,))
        conn.commit()
        conn.close()
...
async def save_bracelet(msg: types.Message, state: FSMContext):
    if msg.photo:
        ...
        conn = get_db()
        c = conn.cursor()
        c.execute('INSERT INTO bracelets (name, desc, price, image_url, created_at) VALUES (%s, %s, %s, %s, %s)',
...
async def diag_notes(msg: types.Message, state: FSMContext):
    ...
    c = conn.cursor()
    try:
        c.execute("INSERT INTO diagnostics (user_id, photo_count, notes, created_at, photo1_file_id, photo2_file_id) VALUES (%s, %s, %s, %s, %s, %s)",
...
async def show_subcategory(cb: types.CallbackQuery):
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute('SELECT name, emoji FROM subcategories WHERE id = %s', (subcat_id,))
    subcat = c.fetchone()
    ...
    # Потом проверяю контент
    c.execute('SELECT title, desc FROM content WHERE cat_id = %s', (subcat_id,))
    content = c.fetchall()
    conn.close()
    ...
    conn2 = get_db(); c2 = conn2.cursor()
    c2.execute("SELECT parent_id FROM subcategories WHERE id=%s", (subcat_id,))
    p = c2.fetchone(); conn2.close()
...
async def show_subsubcategory(cb: types.CallbackQuery):
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute('SELECT name, emoji FROM subsubcategories WHERE id = %s', (subsubcat_id,))
    subsubcat = c.fetchone()
    c.execute('SELECT title, desc FROM content WHERE cat_id = %s', (subsubcat_id,))
    content = c.fetchall()
    conn.close()
    ...
    conn2 = get_db(); c2 = conn2.cursor()
    c2.execute("SELECT parent_id FROM subsubcategories WHERE id=%s", (subsubcat_id,))
    parent = c2.fetchone(); conn2.close()
...
async def view_bracelet(cb: types.CallbackQuery):
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute('SELECT id, name, desc, price, image_url FROM bracelets WHERE id = %s', (bracelet_id,))
    b = c.fetchone()
    conn.close()
...
async def add_to_cart(cb: types.CallbackQuery):
    ...
    c.execute('SELECT id, quantity FROM cart WHERE user_id = %s AND bracelet_id = %s', (user_id, bracelet_id))
    existing = c.fetchone()
    
    if existing:
        c.execute('UPDATE cart SET quantity = quantity + 1 WHERE id = %s', (existing[0],))
    else:
        c.execute('INSERT INTO cart (user_id, bracelet_id, quantity, added_at) VALUES (%s, %s, %s, %s)',
...
async def pay_yandex(cb: types.CallbackQuery):
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute('SELECT SUM(bracelets.price * cart.quantity) FROM cart JOIN bracelets ON cart.bracelet_id = bracelets.id WHERE cart.user_id = %s', (user_id,))
    total = c.fetchone()[0] or 0
    
    # Создаю заказ
    c.execute('INSERT INTO orders (user_id, total_price, status, payment_method, created_at) VALUES (%s, %s, %s, %s, %s)',
...
async def confirm_order(cb: types.CallbackQuery, state: FSMContext):
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute('UPDATE orders SET status = %s WHERE id = %s', ('confirmed', order_id))
    conn.commit()
    conn.close()
...
async def save_review_text(msg: types.Message, state: FSMContext):
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute('SELECT bracelet_id FROM cart WHERE user_id = %s LIMIT 1', (user_id,))
    result = c.fetchone()
    bracelet_id = result[0] if result else 1
    
    c.execute('INSERT INTO reviews (user_id, bracelet_id, rating, text, created_at) VALUES (%s, %s, %s, %s, %s)',
...
async def show_reviews(cb: types.CallbackQuery):
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute('SELECT rating, text FROM reviews WHERE bracelet_id = %s ORDER BY created_at DESC LIMIT 10', (bracelet_id,))
    reviews = c.fetchall()
    conn.close()
...
async def my_referral(cb: types.CallbackQuery):
    user_id = cb.from_user.id
    conn = get_db(); c = conn.cursor()
    c.execute('SELECT referral_count, balance, total_earned FROM referral_balance WHERE user_id = %s', (user_id,))
...
async def admin_broadcast_send(cb: types.CallbackQuery, state: FSMContext):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("INSERT INTO broadcasts (text, created_at, sent_count) VALUES (%s, %s, %s)", (text, datetime.now(), sent))
    conn.commit(); conn.close()
    await cb.message.edit_text(
...
async def add_story_photo(msg: types.Message, state: FSMContext):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("INSERT INTO stories (user_id, text, photo_file_id, approved, created_at) VALUES (%s, %s, %s, FALSE, %s)",
...
async def story_no_photo(cb: types.CallbackQuery, state: FSMContext):
    data = await state.get_data()
    conn = get_db(); c = conn.cursor()
    c.execute("INSERT INTO stories (user_id, text, photo_file_id, approved, created_at) VALUES (%s, %s, NULL, FALSE, %s)",
...
async def approve_story(cb: types.CallbackQuery):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("UPDATE stories SET approved = TRUE WHERE user_id = %s AND approved = FALSE", (user_id,))
    conn.commit(); conn.close()
...
async def quiz_start(cb: types.CallbackQuery, state: FSMContext):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("INSERT INTO quiz_started (user_id, started_at, completed) VALUES (%s,%s,%s) ON CONFLICT (user_id) DO UPDATE SET started_at=EXCLUDED.started_at",
...
async def qz_result(cb: types.CallbackQuery, state: FSMContext):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("INSERT INTO quiz_results (user_id, answers, recommended_stone, created_at) VALUES (%s,%s,%s,%s)",
...
async def stone_detail(cb: types.CallbackQuery):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT stone_name, emoji, properties, chakra, photo_file_id, short_desc, full_desc, color, price_per_bead, forms, notes FROM knowledge WHERE id=%s", (stone_id,))
...
async def ka_photo(msg: types.Message, state: FSMContext):
    ...
    photo_id = msg.photo[-1].file_id if msg.photo else None
    conn = get_db(); c = conn.cursor()
    c.execute("INSERT INTO knowledge (stone_name, emoji, properties, elements, zodiac, chakra, photo_file_id, created_at) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)",
...
async def knowledge_delete(cb: types.CallbackQuery):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT stone_name, emoji FROM knowledge WHERE id=%s", (sid,)); s = c.fetchone()
    c.execute("DELETE FROM knowledge WHERE id=%s", (sid,)); conn.commit(); conn.close()
...
async def my_orders(cb: types.CallbackQuery):
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT id, total_price, status, created_at FROM orders WHERE user_id=%s ORDER BY created_at DESC LIMIT 10", (cb.from_user.id,))
...
async def order_change_status(cb: types.CallbackQuery):
    ...
    order_id = int(cb.data.split("_")[-1])
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT o.id, u.first_name, o.total_price, o.status FROM orders o LEFT JOIN users u ON o.user_id=u.user_id WHERE o.id=%s", (order_id,))
...
async def set_order_status(cb: types.CallbackQuery):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("UPDATE orders SET status=%s WHERE id=%s", (real_status, order_id))
...
async def knowledge_search_results(msg: types.Message, state: FSMContext):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT id, emoji, stone_name FROM knowledge WHERE lower(stone_name) LIKE %s OR lower(properties) LIKE %s OR lower(tasks) LIKE %s ORDER BY stone_name",
...
async def bracelet_filter_results(cb: types.CallbackQuery):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT id, name, desc, price, image_url FROM bracelets WHERE lower(name) LIKE %s OR lower(desc) LIKE %s ORDER BY created_at DESC",
        ...
        try:
            ...
            for (uid,) in users:
                try:
                    ...
                    c.execute("INSERT INTO diag_reminded (user_id, reminded_at) VALUES (%s,%s) ON CONFLICT (user_id) DO UPDATE SET reminded_at=EXCLUDED.reminded_at", (uid, datetime.now()))
                    conn.commit()
        ...
        try:
            ...
            for (did, uid) in rows:
                try:
                    ...
                    c.execute("UPDATE diagnostics SET followup_sent=1 WHERE id=%s", (did,))
                    conn.commit()
                    await asyncio.sleep(0.3)
                except: pass
            conn.close()
...
async def play_track(cb: types.CallbackQuery):
    track_id = int(cb.data.split("_")[-1])
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT name, desc, duration, audio_url FROM music WHERE id=%s", (track_id,))
    t = c.fetchone(); conn.close()
    if not t:
    ...
    try:
        ...
        await cb.message.edit_text(f"❌ Ошибка воспроизведения: {e}", reply_markup=kb)
    await cb.answer()
...
async def showcase_collection(cb: types.CallbackQuery):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT name, emoji, desc FROM showcase_collections WHERE id=%s", (col_id,))
    col = c.fetchone()
    c.execute("SELECT id, name, desc, price, image_file_id FROM showcase_items WHERE collection_id=%s ORDER BY sort_order ASC, created_at ASC", (col_id,))
    items = c.fetchall(); conn.close()
    if not col:
...
async def showcase_item(cb: types.CallbackQuery):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT si.name, si.desc, si.price, si.image_file_id, sc.id FROM showcase_items si JOIN showcase_collections sc ON si.collection_id=sc.id WHERE si.id=%s", (item_id,))
    item = c.fetchone(); conn.close()
    if not item:
        await cb.answer("Не найдено", show_alert=True); return
...
async def sc_add_to_cart(cb: types.CallbackQuery):
    item_id = int(cb.data.split("_")[-1])
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT name, price FROM showcase_items WHERE id=%s", (item_id,))
    item = c.fetchone()
    if not item:
...
async def sc_add_col(cb: types.CallbackQuery, state: FSMContext):
    if not is_admin(cb.from_user.id): await cb.answer("❌", show_alert=True); return
    await cb.message.edit_text("➕ НОВАЯ КОЛЛЕКЦИЯ\n\nВведи название коллекции:")
...
async def sc_save_col_desc(msg: types.Message, state: FSMContext):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("INSERT INTO showcase_collections (name, emoji, desc, created_at) VALUES (%s,%s,%s,%s)",
...
async def sc_admin_collection(cb: types.CallbackQuery):
    ...
    col_id = int(cb.data.split("_")[-1])
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT name, emoji FROM showcase_collections WHERE id=%s", (col_id,))
    col = c.fetchone()
    c.execute("SELECT id, name, price FROM showcase_items WHERE collection_id=%s ORDER BY sort_order ASC, created_at ASC", (col_id,))
    items = c.fetchall(); conn.close()
...
async def sc_add_item(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text("➕ НОВЫЙ ТОВАР\n\nВведи название товара:")
...
async def sc_save_item_photo(msg: types.Message, state: FSMContext):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("INSERT INTO showcase_items (collection_id, name, desc, price, image_file_id, created_at) VALUES (%s,%s,%s,%s,%s,%s)",
              (data['sc_col_id'], data['item_name'], data.get('item_desc',''),
               data.get('item_price'), image_id, __import__('datetime').datetime.now()))
    conn.commit(); conn.close()
...
async def sc_admin_item(cb: types.CallbackQuery):
    ...
    item_id = int(cb.data.split("_")[-1])
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT si.id, si.name, si.desc, si.price, si.image_file_id, si.collection_id FROM showcase_items si WHERE si.id=%s", (item_id,))
    item = c.fetchone(); conn.close()
    if not item: await cb.answer("Не найдено", show_alert=True); return
...
async def sc_save_edit(msg: types.Message, state: FSMContext):
    ...
    conn = get_db(); c = conn.cursor()
    if field == 'name':
        c.execute("UPDATE showcase_items SET name=%s WHERE id=%s", (msg.text, item_id))
    elif field == 'desc':
        c.execute("UPDATE showcase_items SET desc=%s WHERE id=%s", (msg.text, item_id))
    elif field == 'price':
        try: price = float(msg.text.replace(',','.'))
        except: price = None
        c.execute("UPDATE showcase_items SET price=%s WHERE id=%s", (price, item_id))
    elif field == 'photo':
        if msg.photo:
            c.execute("UPDATE showcase_items SET image_file_id=%s WHERE id=%s", (msg.photo[-1].file_id, item_id))
    c.execute("SELECT collection_id FROM showcase_items WHERE id=%s", (item_id,))
    r = c.fetchone(); conn.commit(); conn.close()
...
async def sc_del_item(cb: types.CallbackQuery):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("DELETE FROM showcase_items WHERE id=%s", (item_id,))
    conn.commit(); conn.close()
...
async def sc_del_col(cb: types.CallbackQuery):
    ...
    col_id = int(cb.data.split("_")[-1])
    conn = get_db(); c = conn.cursor()
    c.execute("DELETE FROM showcase_items WHERE collection_id=%s", (col_id,))
    c.execute("DELETE FROM showcase_collections WHERE id=%s", (col_id,))
    conn.commit(); conn.close()
...
async def add_workout_process(msg: types.Message, state: FSMContext):
    ...
    c = conn.cursor()
    try:
        c.execute("INSERT INTO workouts (name, desc, duration, difficulty, created_at) VALUES (%s, %s, %s, %s, %s)",
...
async def main():
    ...
    print(f"🔍 DATABASE_URL = {repr(os.environ.get('DATABASE_URL', 'НЕ НАЙДЕНА'))[:80]}")
    init_db()
    print('✅ БД готова')
