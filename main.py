...
if not os.getenv("BOT_TOKEN"):
    logger.warning("⚠️ BOT_TOKEN не установлен - бот не запустится")
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS referrals
                 (id INTEGER PRIMARY KEY AUTOINCREMENT, referrer_id INT, referred_id INT, created_at TIMESTAMP)''')
    c.execute('''CREATE TABLE IF NOT EXISTS referral_balance
                 (user_id INT PRIMARY KEY, balance REAL DEFAULT 0, total_earned REAL DEFAULT 0, referral_count INT DEFAULT 0)''')

    # Истории клиентов
    c.execute('''CREATE TABLE IF NOT EXISTS stories
                 (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INT, text TEXT, photo_file_id TEXT, approved BOOLEAN DEFAULT FALSE, created_at TIMESTAMP)''')

    # Рассылки
    c.execute('''CREATE TABLE IF NOT EXISTS broadcasts
                 (id INTEGER PRIMARY KEY AUTOINCREMENT, text TEXT, created_at TIMESTAMP, sent_count INT DEFAULT 0)''')

    conn.commit()

    # Новые колонки если их нет
    for col_sql in [
        "ALTER TABLE users ADD COLUMN welcome_sent BOOLEAN DEFAULT FALSE",
        "ALTER TABLE users ADD COLUMN referred_by INT DEFAULT NULL",
        "ALTER TABLE users ADD COLUMN last_diag_started TIMESTAMP",
    ]:
        try:
            c.execute(col_sql)
            conn.commit()
    ...
    if is_new:
        ...
        if ref_id:
            ...
            if c.fetchone():
                ...
                c.execute("INSERT INTO referral_balance (user_id, referral_count) VALUES (?, 1) ON CONFLICT(user_id) DO UPDATE SET referral_count = referral_count + 1",
                          (ref_id,))
                conn.commit()
                try:
                    await bot.send_message(ref_id, "🎉 По вашей реферальной ссылке зарегистрировался новый пользователь!")
                except:
                    pass
    else:
...
async def send_to_master_finish(msg: types.Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer("✅ Сообщение отправлено мастеру! Ожидайте ответа.")
    except Exception as e:
...
async def my_referral(cb: types.CallbackQuery):
    ...
    text = (
        ...
        f"16+ друзей → 15% + статус 👑 Амбассадор + скидка 15%"
    )
    await cb.message.edit_text(text, reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
        [types.InlineKeyboardButton(text="← НАЗАД", callback_data="menu")],
    ]))
...
async def admin_stats(cb: types.CallbackQuery):
    ...
    c.execute("SELECT COALESCE(SUM(total_price), 0) FROM orders WHERE status = 'confirmed' AND DATE(created_at) = DATE('now')")
...
async def admin_broadcast_start(cb: types.CallbackQuery, state: FSMContext):
    if not is_admin(cb.from_user.id):
        await cb.answer("❌ Нет прав!", show_alert=True)
        return
    await cb.message.edit_text("📢 РАССЫЛКА\n\nНапишите текст сообщения для всех пользователей:")
...
async def show_stories_cb(cb: types.CallbackQuery):
    conn = get_db()
    c = conn.cursor()
    c.execute('''SELECT s.text, s.photo_file_id, u.first_name
                 FROM stories s JOIN users u ON s.user_id = u.user_id
                 WHERE s.approved = TRUE ORDER BY s.created_at DESC LIMIT 5''')
    stories = c.fetchall()
    conn.close()
    if not stories:
        await cb.message.edit_text(
...
async def add_story_text(msg: types.Message, state: FSMContext):
    ...
    await msg.answer(
        "📸 Прикрепите фото (необязательно) или нажмите кнопку:",
        reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
