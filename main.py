...
async def show_subcategory(cb: types.CallbackQuery):
    ...
    conn2 = get_db(); c2 = conn2.cursor()
    c2.execute("SELECT parent_id FROM subcategories WHERE id=?", (subcat_id,))
...
async def show_subsubcategory(cb: types.CallbackQuery):
    ...
    conn2 = get_db(); c2 = conn2.cursor()
    c2.execute("SELECT parent_id FROM subsubcategories WHERE id=?", (subsubcat_id,))
...
async def sc_save_item_photo(msg: types.Message, state: FSMContext):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("INSERT INTO showcase_items (collection_id, name, desc, price, image_file_id, created_at) VALUES (?,?,?,?,?,?)",
