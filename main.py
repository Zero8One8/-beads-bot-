...
def init_db():
    ...
    # НОВАЯ ТАБЛИЦА ДЛЯ АКТИВНЫХ ПРОМОКОДОВ (исправление БАГА #2)
    c.execute('''CREATE TABLE IF NOT EXISTS active_promocodes
...
async def pay_yandex(cb: types.CallbackQuery, state: FSMContext):
    ...
    # Сначала пробуем получить промокод из active_promocodes (БД)
    c.execute("SELECT code, discount_pct, discount_rub FROM active_promocodes WHERE user_id=?", (user_id,))
    active = c.fetchone()
    if active:
        promo_code = active[0]
        promo_pct = active[1]
        ...
        # Если нет в БД, пробуем из state (для обратной совместимости)
        try:
    ...
    if promo_code:
        ...
        # Удаляем из активных, чтобы нельзя было применить повторно
        c.execute("DELETE FROM active_promocodes WHERE user_id=?", (user_id,))
...
async def confirm_order(cb: types.CallbackQuery, state: FSMContext):
    ...
    c.execute("DELETE FROM active_promocodes WHERE user_id=?", (cb.from_user.id,))
...
async def check_promo(msg: types.Message, state: FSMContext):
    ...
    # Проверял ли этот пользователь уже этот промокод
    ...
    # Сохраняем в БД active_promocodes
    c.execute("INSERT OR REPLACE INTO active_promocodes (user_id, code, discount_pct, discount_rub, applied_at) VALUES (?,?,?,?,?)",
              (user_id, code, dpct, drub, datetime.now()))
