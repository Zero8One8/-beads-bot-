...
def init_db():
    ...
    conn.commit()
    for _sql in ["ALTER TABLE users ADD COLUMN welcome_sent BOOLEAN DEFAULT FALSE","ALTER TABLE users ADD COLUMN referred_by INT DEFAULT NULL","ALTER TABLE diagnostics ADD COLUMN followup_sent INT DEFAULT 0"]:
...
async def diag_start_cb(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(
        "🔮 ДИАГНОСТИКА\n\nОтветьте на несколько вопросов — мастер подберёт камни лично для вас.\n\nШаг 1 из 4 — Кто вы?",
...
async def stone_detail(cb: types.CallbackQuery):
    ...
    try:
        if photo_id: await cb.message.answer_photo(photo=photo_id, caption=text, reply_markup=kb); await cb.message.delete()
        else: await cb.message.edit_text(text, reply_markup=kb)
    except: await cb.message.edit_text(text, reply_markup=kb)
    await cb.answer()
...
async def knowledge_search_results(msg: types.Message, state: FSMContext):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT id, emoji, stone_name FROM knowledge WHERE lower(stone_name) LIKE ? OR lower(properties) LIKE ? OR lower(elements) LIKE ? ORDER BY stone_name",
