...
async def diag_start_cb(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(
        "🔮 ДИАГНОСТИКА\n\nДля точного подбора камней ответьте на несколько вопросов.\n\nШаг 1 из 4 — Кто вы?",
        reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
            [types.InlineKeyboardButton(text="👩 Женщина", callback_data="dg_gender_f")],
            [types.InlineKeyboardButton(text="👨 Мужчина", callback_data="dg_gender_m")],
            [types.InlineKeyboardButton(text="← НАЗАД", callback_data="menu")],
        ])
    )
    await cb.answer()
...
async def dg_gender(cb: types.CallbackQuery, state: FSMContext):
    ...
    if gender == "f":
        await cb.message.edit_text(
            "Шаг 2 из 4 — Что сейчас ближе всего к вашему состоянию?",
            reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
...
async def dg_q1_answer(cb: types.CallbackQuery, state: FSMContext):
    ...
    if dg_type == "f_lost":
        await cb.message.edit_text(
            "Шаг 4 из 4 — Как давно вам так?",
            reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
                [types.InlineKeyboardButton(text="🗓 Недавно — ещё острая боль", callback_data="dg_q2_recent")],
                [types.InlineKeyboardButton(text="📆 Несколько месяцев — вроде живу, но не живу", callback_data="dg_q2_months")],
                [types.InlineKeyboardButton(text="🗃 Больше года — уже забыла какой была раньше", callback_data="dg_q2_year")],
                [types.InlineKeyboardButton(text="❓ Давно — не помню себя лёгкой и счастливой", callback_data="dg_q2_always")],
            ])
        )
    elif dg_type == "f_grow":
        await cb.message.edit_text(
            "Шаг 4 из 4 — Что сейчас мешает идти быстрее?",
            reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
                [types.InlineKeyboardButton(text="⚡ Нет энергии — быстро выгораю", callback_data="dg_q2_energy")],
                [types.InlineKeyboardButton(text="🌀 Сомнения и внутренние блоки", callback_data="dg_q2_blocks")],
                [types.InlineKeyboardButton(text="👁 Чужое влияние — люди тянут назад", callback_data="dg_q2_people")],
                [types.InlineKeyboardButton(text="🌫 Что-то держит, не могу понять что", callback_data="dg_q2_unknown")],
            ])
        )
    elif dg_type == "m_down":
        await cb.message.edit_text(
            "Шаг 4 из 4 — Как давно это длится?",
            reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
                [types.InlineKeyboardButton(text="🗓 Недавно — конкретный удар выбил из колеи", callback_data="dg_q2_recent")],
                [types.InlineKeyboardButton(text="📆 Несколько месяцев — постепенно накапливалось", callback_data="dg_q2_months")],
                [types.InlineKeyboardButton(text="🗃 Больше года — уже норма, но внутри пусто", callback_data="dg_q2_year")],
                [types.InlineKeyboardButton(text="❓ Всегда так — никогда не было по-настоящему хорошо", callback_data="dg_q2_always")],
            ])
        )
    else:  # m_up
        await cb.message.edit_text(
            "Шаг 4 из 4 — Что сейчас мешает расти быстрее?",
            reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
                [types.InlineKeyboardButton(text="🌀 Внутренние сомнения и страх ошибки", callback_data="dg_q2_doubt")],
                [types.InlineKeyboardButton(text="👥 Люди вокруг тянут или предают", callback_data="dg_q2_people")],
                [types.InlineKeyboardButton(text="⚡ Энергия есть, но быстро выгораю", callback_data="dg_q2_burnout")],
                [types.InlineKeyboardButton(text="🌫 Что-то невидимое тормозит — не пойму что", callback_data="dg_q2_unknown")],
            ])
        )
    await cb.answer()

# ── ВЕТКА: ЖЕНЩИНА РАЗВИВАЕТСЯ ───────────────────────────────────────────
...
async def dg_q2_answer(cb: types.CallbackQuery, state: FSMContext):
    await state.update_data(dg_q2=cb.data)
    await cb.message.edit_text(
        "✅ Отлично! Картина складывается.\n\n"
        "Теперь последний шаг — загрузите две фотографии.\n\n"
        "📸 Фото 1: ваши ладони (мастер читает по ним очень много)\n"
        "📸 Фото 2: ваше лицо или любое фото которое вы хотите показать\n\n"
        "На основе ответов и фото мастер подберёт камни лично для вас.\n"
        "Ответ в течение 24 часов. Загружайте первое фото 👇"
    )
    await state.set_state(DiagnosticStates.waiting_photo1)
    await cb.answer()
...
# ═══════════════════════════════════════════════════════════════════════════
# ТЕСТ "КАКОЙ КАМЕНЬ ПОДХОДИТ ИМЕННО ВАМ" — ПОЛНАЯ ВОРОНКА
# ═══════════════════════════════════════════════════════════════════════════

QUIZ_STONES = {
    # Женщина потеряла себя
    ...
    ("f_lost", "numb",  "year"):     ("Цитрин",         "💛", "Камень радости и тепла",             "Он будет напоминать вам каждый день — жизнь прекрасна"),
    ...
    ("m_down", "collapsed","year"):     ("Гранат",      "❤️", "Камень возрождения",                 "Он разожжёт огонь в том, что казалось потухшим навсегда"),
    ...
    ("m_up", "clarity",  "burnout"):  ("Аквамарин",     "💙","Камень холодного разума",             "Он охладит голову и вернёт способность мыслить стратегически"),
    ...
    ("m_up", "strength", "people"):   ("Чёрный турмалин","🖤","Камень неуязвимости",               "Он сделает вас недосягаемым для чужих атак и манипуляций"),
    ...
    ("m_up", "shield",   "burnout"):  ("Лабрадор",      "🔵","Камень невидимости для врагов",      "Он скроет ваши планы и действия от недоброжелателей"),
    ("m_up", "shield",   "unknown"):  ("Обсидиан",      "⚫","Камень глубинной защиты",            "Он найдёт и нейтрализует источник угрозы"),
}

def get_quiz_stone(dg_type, q1, q2):
    q1_key = q1.replace("dg_q1_", "")
    q2_key = q2.replace("dg_q2_", "")
    key = (dg_type, q1_key, q2_key)
    default = ("Горный хрусталь", "🔮", "Камень чистоты и усиления", "Он усилит любой запрос и очистит путь к цели")
    return QUIZ_STONES.get(key, default)
...
async def quiz_start(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(
        ...
        "5 вопросов. Отвечайте честно — чем точнее ответы, тем точнее результат.\n\n"
        "Шаг 1 из 5 — Кто вы?",
        reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
            [types.InlineKeyboardButton(text="👩 Женщина", callback_data="qz_gender_f")],
            [types.InlineKeyboardButton(text="👨 Мужчина", callback_data="qz_gender_m")],
            [types.InlineKeyboardButton(text="← НАЗАД", callback_data="menu")],
        ])
    )
    await cb.answer()

@main_router.callback_query(F.data.startswith("qz_gender_"))
async def qz_gender(cb: types.CallbackQuery, state: FSMContext):
    gender = cb.data.split("_")[-1]
    await state.update_data(qz_gender=gender)
    if gender == "f":
        await cb.message.edit_text(
            "Шаг 2 из 5 — Что сейчас ближе к вашему состоянию?",
            reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
                [types.InlineKeyboardButton(text="💔 Больно — что-то важное ушло или рассыпалось", callback_data="qz_type_f_lost")],
                [types.InlineKeyboardButton(text="🌱 Развиваюсь — ищу себя и свой путь", callback_data="qz_type_f_grow")],
            ])
        )
    else:
        await cb.message.edit_text(
            "Шаг 2 из 5 — Что сейчас ближе к вашему состоянию?",
            reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
                [types.InlineKeyboardButton(text="🌑 Всё навалилось — не вывожу", callback_data="qz_type_m_down")],
                [types.InlineKeyboardButton(text="⚡ Рвусь вперёд — нужна дополнительная сила", callback_data="qz_type_m_up")],
            ])
        )
    await cb.answer()

# ── ШАГ 3 по каждой ветке ────────────────────────────────────────────────

@main_router.callback_query(F.data == "qz_type_f_lost")
async def qz_f_lost_q3(cb: types.CallbackQuery, state: FSMContext):
    await state.update_data(qz_type="f_lost")
    await cb.message.edit_text(
        "Шаг 3 из 5 — Что сейчас внутри?",
        reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
...
@main_router.callback_query(F.data == "qz_type_f_grow")
async def qz_f_grow_q3(cb: types.CallbackQuery, state: FSMContext):
    await state.update_data(qz_type="f_grow")
    await cb.message.edit_text(
        "Шаг 3 из 5 — Что сейчас важнее всего усилить?",
        reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
...
@main_router.callback_query(F.data == "qz_type_m_down")
async def qz_m_down_q3(cb: types.CallbackQuery, state: FSMContext):
    await state.update_data(qz_type="m_down")
    await cb.message.edit_text(
        "Шаг 3 из 5 — Что сейчас происходит?",
        reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
...
@main_router.callback_query(F.data == "qz_type_m_up")
async def qz_m_up_q3(cb: types.CallbackQuery, state: FSMContext):
    await state.update_data(qz_type="m_up")
    await cb.message.edit_text(
        "Шаг 3 из 5 — Что хотите усилить?",
        reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
...
async def qz_q3_answer(cb: types.CallbackQuery, state: FSMContext):
    await state.update_data(qz_q3=cb.data)
    data = await state.get_data()
    qz_type = data.get("qz_type", "")

    if qz_type in ("f_lost",):
        await cb.message.edit_text(
            "Шаг 4 из 5 — Бывает ли такое ощущение — вы стараетесь, а всё как в стену?",
            reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
                [types.InlineKeyboardButton(text="🪨 Да постоянно — как будто что-то блокирует", callback_data="qz_q4_wall")],
                [types.InlineKeyboardButton(text="🌊 Волнами — то лучше, то снова стопорится", callback_data="qz_q4_waves")],
                [types.InlineKeyboardButton(text="🌫 Есть ощущение чужого взгляда или влияния", callback_data="qz_q4_eyeon")],
                [types.InlineKeyboardButton(text="🌱 Просто нужна точка опоры", callback_data="qz_q4_ground")],
            ])
        )
    elif qz_type in ("f_grow",):
        await cb.message.edit_text(
            "Шаг 4 из 5 — Какой цвет вас притягивает интуитивно?",
            reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
                [types.InlineKeyboardButton(text="💜 Тёмный и глубокий", callback_data="qz_q4_dark")],
                [types.InlineKeyboardButton(text="💚 Природный и живой", callback_data="qz_q4_green")],
                [types.InlineKeyboardButton(text="✨ Светлый и прозрачный", callback_data="qz_q4_light")],
                [types.InlineKeyboardButton(text="❤️ Тёплый и огненный", callback_data="qz_q4_warm")],
            ])
        )
    elif qz_type in ("m_down",):
        await cb.message.edit_text(
            "Шаг 4 из 5 — Бывает ощущение что сколько бы ни вкладывал — всё уходит в никуда?",
            reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
                [types.InlineKeyboardButton(text="🪨 Да — как дыра, деньги энергия время всё исчезает", callback_data="qz_q4_drain")],
                [types.InlineKeyboardButton(text="🎯 Стараюсь, но что-то постоянно сбивает", callback_data="qz_q4_sabotage")],
                [types.InlineKeyboardButton(text="👤 Есть люди рядом которые тянут вниз", callback_data="qz_q4_vampires")],
                [types.InlineKeyboardButton(text="🌫 Просто нет почвы под ногами", callback_data="qz_q4_noground")],
            ])
        )
    else:  # m_up
        await cb.message.edit_text(
            "Шаг 4 из 5 — Что для вас важнее всего?",
            reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
                [types.InlineKeyboardButton(text="🏆 Стать первым в своей сфере", callback_data="qz_q4_first")],
                [types.InlineKeyboardButton(text="💎 Достаток и стабильность для семьи", callback_data="qz_q4_family")],
                [types.InlineKeyboardButton(text="🔥 Оставаться в ресурсе надолго", callback_data="qz_q4_resource")],
                [types.InlineKeyboardButton(text="🌟 Реализовать то для чего пришёл в этот мир", callback_data="qz_q4_destiny")],
            ])
        )
    await cb.answer()
...
async def qz_q4_answer(cb: types.CallbackQuery, state: FSMContext):
    await state.update_data(qz_q4=cb.data)
    data = await state.get_data()
    qz_type = data.get("qz_type", "")

    if qz_type in ("f_lost", "f_grow"):
        await cb.message.edit_text(
            "Шаг 5 из 5 — Чего вы хотите прямо сейчас — честно?",
            reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
                [types.InlineKeyboardButton(text="🔥 Снова почувствовать себя желанной и живой", callback_data="qz_q5_alive")],
                [types.InlineKeyboardButton(text="🛡 Защититься — от боли, людей, чужого влияния", callback_data="qz_q5_protect")],
                [types.InlineKeyboardButton(text="💫 Отпустить прошлое и двигаться дальше", callback_data="qz_q5_release")],
                [types.InlineKeyboardButton(text="🤍 Просто внутреннего покоя", callback_data="qz_q5_peace")],
            ])
        )
    else:
        await cb.message.edit_text(
            "Шаг 5 из 5 — Чего вы хотите прямо сейчас — честно?",
            reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
                [types.InlineKeyboardButton(text="⚡ Вернуть энергию и желание действовать", callback_data="qz_q5_energy")],
                [types.InlineKeyboardButton(text="🛡 Защититься — от людей, потерь, влияния", callback_data="qz_q5_protect")],
                [types.InlineKeyboardButton(text="💰 Сдвинуть деньги и дела с мёртвой точки", callback_data="qz_q5_money")],
                [types.InlineKeyboardButton(text="🧭 Найти направление — понять куда идти", callback_data="qz_q5_direction")],
            ])
        )
    await cb.answer()
...
async def qz_result(cb: types.CallbackQuery, state: FSMContext):
    await state.update_data(qz_q5=cb.data)
    data = await state.get_data()
    dg_type = data.get("qz_type", "f_lost")
    q3 = data.get("qz_q3", "").replace("qz_q3_", "")
    q4 = data.get("qz_q4", "").replace("qz_q4_", "")

    # Используем q3 и q4 как ключи
    key = (dg_type, q3, q4)
    default = ("Горный хрусталь", "🔮", "Камень чистоты и усиления", "Он усилит любой запрос и очистит путь к цели")
    stone, emoji, desc, why = QUIZ_STONES.get(key, default)

    conn = get_db()
    c = conn.cursor()
    c.execute("INSERT INTO quiz_results (user_id, answers, recommended_stone, created_at) VALUES (?,?,?,?)",
              (cb.from_user.id, f"{dg_type}|{q3}|{q4}", stone, datetime.now()))
    conn.commit()
    conn.close()
    ...
    await cb.message.edit_text(
        ...
        f"{desc}\n\n"
        f"✨ {why}\n\n"
        f"Это результат теста. Для точного персонального подбора — пройдите диагностику по фото.\n"
        f"Мастер изучит ваш запрос лично и подберёт именно то что нужно вам.",
        reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
...
async def showcase_bracelets(cb: types.CallbackQuery):
    ...
    for item in items:
        ...
        kb = types.InlineKeyboardMarkup(inline_keyboard=[
            ...
            [types.InlineKeyboardButton(text="🔍 ПОДРОБНЕЕ", callback_data=f"view_bracelet_{bid}")],
        ])
        try:
            if image_url:
                await cb.message.answer_photo(photo=image_url, caption=caption, reply_markup=kb)
            else:
...
# ═══════════════════════════════════════════════════════════════════════════
# ТЕСТ "КАКОЙ КАМЕНЬ ПОДХОДИТ ВАМ"
# ═══════════════════════════════════════════════════════════════════════════

QUIZ_QUESTIONS = [
    {"text": "🔮 ТЕСТ: КАКОЙ КАМЕНЬ ПОДХОДИТ ИМЕННО ВАМ?\n\nВопрос 1 из 5\n\nЧто вас беспокоит больше всего?",
     "options": [("💔 Отношения и любовь","love"),("💼 Работа и деньги","money"),("😔 Стресс и тревога","stress"),("⚡ Нет энергии","energy")]},
    {"text": "Вопрос 2 из 5\n\nВаше главное желание на ближайший год?",
     "options": [("🏠 Стабильность и защита","stability"),("🌟 Успех и признание","success"),("❤️ Любовь и семья","love"),("🧘 Покой и гармония","harmony")]},
    {"text": "Вопрос 3 из 5\n\nКакой цвет вас притягивает?",
     "options": [("💜 Фиолетовый / синий","purple"),("💚 Зелёный / бирюзовый","green"),("❤️ Красный / оранжевый","red"),("🤍 Белый / прозрачный","white")]},
    {"text": "Вопрос 4 из 5\n\nКакая стихия вам ближе?",
     "options": [("🔥 Огонь","fire"),("💧 Вода","water"),("🌍 Земля","earth"),("💨 Воздух","air")]},
    {"text": "Вопрос 5 из 5\n\nКак вы чаще всего себя чувствуете?",
     "options": [("😤 Раздражён, нужна защита","protection"),("😢 Грустно, нужна поддержка","support"),("😰 Тревожно, нужен покой","calm"),("😴 Нет мотивации","motivation")]},
]

STONE_MAP = {
    "love": ("Розовый кварц","💗","Камень любви","Откроет ваше сердце и притянет гармоничные отношения"),
    "money": ("Цитрин","💛","Камень изобилия","Усилит вашу финансовую энергию и привлечёт процветание"),
    "stress": ("Аметист","💜","Камень спокойствия","Снимет тревогу и наполнит внутренним миром"),
    "energy": ("Гранат","❤️","Камень силы","Зарядит энергией и вернёт страсть к жизни"),
    "stability": ("Чёрный турмалин","🖤","Камень защиты","Создаст прочный энергетический щит"),
    "success": ("Тигровый глаз","🟠","Камень успеха","Усилит харизму и притянет нужных людей"),
    "harmony": ("Лунный камень","🤍","Камень гармонии","Настроит на поток и раскроет внутреннюю мудрость"),
    "purple": ("Аметист","💜","Камень духовности","Усилит интуицию и защитит от негатива"),
    "green": ("Малахит","💚","Камень трансформации","Поможет изменить жизнь к лучшему"),
    "red": ("Гранат","❤️","Камень страсти","Пробудит вашу жизненную силу"),
    "white": ("Горный хрусталь","🔮","Камень чистоты","Очистит ауру и усилит любой другой камень"),
    "fire": ("Сердолик","🔶","Камень творчества","Зажжёт вашу внутреннюю искру"),
    "water": ("Аквамарин","💙","Камень интуиции","Усилит чувствительность и ясновидение"),
    "earth": ("Яшма","🟫","Камень стабильности","Даст прочную основу во всех сферах жизни"),
    "air": ("Лазурит","💙","Камень мудрости","Откроет разум для новых идей"),
    "protection": ("Обсидиан","⚫","Камень защиты","Создаст мощный щит от негатива и сглаза"),
    "support": ("Розовый кварц","💗","Камень поддержки","Окутает теплом и нежностью"),
    "calm": ("Голубой халцедон","💠","Камень покоя","Успокоит ум и снимет тревогу"),
    "motivation": ("Гранат","❤️","Камень мотивации","Вернёт силы и желание действовать"),
}

def get_stone_rec(answers):
    counts = {}
    for a in answers:
        counts[a] = counts.get(a, 0) + 1
    dominant = max(counts, key=counts.get) if counts else "harmony"
    return STONE_MAP.get(dominant, STONE_MAP["harmony"])
...
async def ka_set_name(msg: types.Message, state: FSMContext):
    await state.update_data(stone_name=msg.text)
    await msg.answer("Введите эмодзи (например: 💜):")
    await state.set_state(KnowledgeAdminStates.stone_emoji)

@admin_router.message(KnowledgeAdminStates.stone_photo)
async def ka_set_photo(msg: types.Message, state: FSMContext):
    data = await state.get_data()
    photo_id = msg.photo[-1].file_id if msg.photo else None
    conn = get_db(); c = conn.cursor()
    c.execute("INSERT INTO knowledge (stone_name, emoji, properties, elements, zodiac, chakra, photo_file_id, created_at) VALUES (?,?,?,?,?,?,?,?)",
              (data["stone_name"], data["stone_emoji"], data["stone_properties"],
               data["stone_elements"], data["stone_zodiac"], data["stone_chakra"], photo_id, datetime.now()))
    conn.commit(); conn.close()
    await msg.answer(
        f"✅ {data['stone_emoji']} {data['stone_name']} добавлен в базу знаний!",
        reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
...
async def knowledge_admin_list(cb: types.CallbackQuery):
    ...
    stones = c.fetchall(); conn.close()
    if not stones:
        await cb.answer("База пустая", show_alert=True); return
    ...
    await cb.message.edit_text("📋 Нажмите на камень чтобы удалить:", reply_markup=types.InlineKeyboardMarkup(inline_keyboard=buttons))
    await cb.answer()
...
async def knowledge_delete(cb: types.CallbackQuery):
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT stone_name, emoji FROM knowledge WHERE id=?", (stone_id,))
    s = c.fetchone()
    c.execute("DELETE FROM knowledge WHERE id=?", (stone_id,))
    conn.commit(); conn.close()
    ...
    stones = c.fetchall(); conn.close()
    if not stones:
        await cb.message.edit_text("База знаний пустая.", reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
            [types.InlineKeyboardButton(text="← НАЗАД", callback_data="admin_knowledge")],
        ])); return
    buttons = [[types.InlineKeyboardButton(text=f"🗑 {s[1]} {s[2]}", callback_data=f"knowledge_del_{s[0]}")] for s in stones]
    buttons.append([types.InlineKeyboardButton(text="← НАЗАД", callback_data="admin_knowledge")])
    await cb.message.edit_text("📋 Нажмите на камень чтобы удалить:", reply_markup=types.InlineKeyboardMarkup(inline_keyboard=buttons))
...
async def admin_quiz_results(cb: types.CallbackQuery):
    ...
    c.execute("SELECT recommended_stone, COUNT(*) as cnt FROM quiz_results GROUP BY recommended_stone ORDER BY cnt DESC LIMIT 5")
    top = c.fetchall(); conn.close()
    text = f"🔮 РЕЗУЛЬТАТЫ ТЕСТА\n\nВсего прошли: {total}\n\n🏆 Топ камней:\n"
    for stone, cnt in top:
        text += f"  {stone}: {cnt} чел.\n"
    await cb.message.edit_text(text, reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
        [types.InlineKeyboardButton(text="← НАЗАД", callback_data="admin_panel")],
    ]))
    await cb.answer()
