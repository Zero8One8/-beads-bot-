...
DB = os.getenv('DB', '/app/data/bot.db')  # Railway путь по умолчанию

db_path = Path(DB).parent
db_path.mkdir(parents=True, exist_ok=True)
# Используем относительные пути от БД
base_storage = db_path / 'storage'
(base_storage / 'diagnostics').mkdir(parents=True, exist_ok=True)
(base_storage / 'stories').mkdir(parents=True, exist_ok=True)
(base_storage / 'photos').mkdir(parents=True, exist_ok=True)
...
def escape_markdown(text: str) -> str:
    """Экранирует специальные символы Markdown для безопасности."""
    if not text:
        return ""
    special_chars = ['_', '*', '[', ']', '(', ')', '~', '`', '>', '#', '+', '-', '=', '|', '{', '}', '.', '!']
...
async def cleanup_rate_cache():
    """Фоновая задача для очистки устаревших записей rate_limit каждый час."""
    while True:
        await asyncio.sleep(3600)
        now = time.time()
        expired_keys = [k for k, v in _rate_cache.items() if now - v > 3600]
        for key in expired_keys:
            del _rate_cache[key]
        if expired_keys:
            logger.info(f"Очищено {len(expired_keys)} устаревших rate_limit записей")
...
def db_connection():
    ...
    conn = sqlite3.connect(DB, timeout=30)  # ✅ FIX: добавлен timeout
    conn.row_factory = sqlite3.Row
    conn.execute("PRAGMA busy_timeout = 30000")   # ✅ FIX: применяется на каждое соединение
    conn.execute("PRAGMA journal_mode=WAL")        # ✅ FIX: WAL на каждое соединение
    conn.execute("PRAGMA synchronous=NORMAL")
    try:
        yield conn
        conn.commit()   # ✅ FIX: автоматический commit при успехе
    except Exception:
        conn.rollback()  # ✅ FIX: rollback при ошибке
        raise
    finally:
        conn.close()

@contextmanager
def db_cursor(max_retries=5):
    """Контекстный менеджер для курсора с автоматическим закрытием и retry при блокировке."""
    for attempt in range(max_retries):
        try:
            with db_connection() as conn:
                cursor = conn.cursor()
                yield cursor
                return  # Успех - выходим (commit выполнит db_connection)
        except sqlite3.OperationalError as e:
            if "database is locked" in str(e) and attempt < max_retries - 1:
                logger.warning(f"БД заблокирована, повтор {attempt + 1}/{max_retries}")
                time.sleep(0.05 * (attempt + 1))  # ✅ FIX: уменьшен, busy_timeout в db_connection
                continue
            raise
...
# ❌ НЕ ВЫЗЫВАЕМ init_db() здесь - он вызовется в main()
...
async def check_pending_orders():
    ...
    while True:
        try:
            ...
            # ✅ FIX: Шаг 1 — только DB операции, никаких await внутри with
            cancelled_orders = []
            with db_connection() as conn:
                c = conn.cursor()
                ...
                for order in old_orders:
                    ...
                    cancelled_orders.append((order_id, user_id))
                
                c.execute("DELETE FROM order_locks WHERE locked_until < datetime('now')")
                # conn.commit() вызовется автоматически при выходе из with
            
            # ✅ FIX: Шаг 2 — async вызовы ПОСЛЕ закрытия соединения
            for order_id, user_id in cancelled_orders:
...
async def check_birthdays():
    ...
    while True:
        try:
            ...
            # ✅ FIX: Шаг 1 — только DB операции, никаких await внутри with
            birthday_notifications = []  # [(user_id, name, promo_code)]
            with db_connection() as conn:
                c = conn.cursor()
                ...
                for user in birthday_users:
                    ...
                    birthday_notifications.append((user_id, name, promo_code))
                # conn.commit() вызовется автоматически при выходе из with
            
            # ✅ FIX: Шаг 2 — async вызовы ПОСЛЕ закрытия соединения
            for user_id, name, promo_code in birthday_notifications:
                try:
                    kb = InlineKeyboardMarkup(inline_keyboard=[
...
# ═══════════════════════════════════════════════════════════════════════════
# НОВЫЕ HANDLERS - ВСЕ ОТСУТСТВУЮЩИЕ ОБРАБОТЧИКИ
# ═══════════════════════════════════════════════════════════════════════════
...
async def knowledge_list(cb: CallbackQuery):
    ...
    with db_cursor() as c:
        c.execute("""SELECT id, stone_name, emoji, properties
                    FROM stones_knowledge
                    ORDER BY stone_name
                    LIMIT 31""")
        stones = c.fetchall()
    
    if not stones:
        # Если база знаний пуста, показываем информационное сообщение
        await safe_edit(cb,
            "📚 *БАЗА ЗНАНИЙ О КАМНЯХ*\n\n"
...
async def set_birthday(cb: CallbackQuery, state: FSMContext):
    ...
    with db_cursor() as c:
        ...
        if user and user['birthday']:
            ...
            need_state = True  # ✅ FIX: флаг вместо await внутри with
    
    if need_state:
        await state.set_state(BirthdayStates.waiting_birthday)  # ✅ FIX: вызов после закрытия with
...
async def admin_funnel(cb: CallbackQuery):
    ...
    with db_cursor() as c:
        stats = {}
        for event_type in FunnelTracker.EVENTS.keys():
            c.execute("SELECT COUNT(DISTINCT user_id) as cnt FROM funnel_stats WHERE event_type = ?",
                      (event_type,))
            stats[event_type] = c.fetchone()['cnt']
...
async def admin_knowledge(cb: CallbackQuery):
    ...
    with db_cursor() as c:
        c.execute("SELECT COUNT(*) as cnt FROM stones_knowledge")
        count = c.fetchone()['cnt']
...
async def export_orders(cb: CallbackQuery):
    ...
    with db_cursor() as c:
        c.execute("""SELECT orders.id, orders.user_id, users.first_name, orders.total_price, 
    ...
    writer.writerow(['ID', 'User ID', 'Name', 'Amount', 'Status', 'Date'])
...
async def start(msg: Message, state: FSMContext):
    ...
    with db_connection() as conn:
        ...
        if is_new:
            ...
            if ref_id:
                ...
                ref_exists = c.fetchone() is not None
            else:
                ref_exists = False
    ...
    # ✅ FIX: соединение с БД закрыто и закоммичено — теперь безопасно делать async вызовы
    
    if is_new and ref_id and ref_exists:
        try:
    ...
    if is_new:
        await FunnelTracker.track(user_id, 'start')  # ✅ FIX: вызывается ПОСЛЕ закрытия conn
...
# ═══════════════════════════════════════════════════════════════════════════
# ФИНАЛЬНЫЕ HANDLERS - ВСЕ ОСТАВШИЕСЯ 32 ФУНКЦИИ
# ═══════════════════════════════════════════════════════════════════════════

# ======================================================
# КОРЗИНА - ОФОРМЛЕНИЕ И ОЧИСТКА
# ======================================================
...
# ======================================================
# ФИЛЬТРЫ ВИТРИНЫ - ВСЕ 5 ВАРИАНТОВ
# ======================================================

@main_router.callback_query(F.data.startswith("filter_price_"))
@rate_limit(2.0)
async def filter_by_price(cb: CallbackQuery):
    """Фильтр по цене."""
    filter_type = cb.data.split("_")[-1]  # 1000, 3000, plus
...
# ═══════════════════════════════════════════════════════════════════════════
# АДМИНСКИЕ HANDLERS - СПИСКИ И ФОРМЫ
# ═══════════════════════════════════════════════════════════════════════════
...
# ======================================================
# АДМИН - СПИСКИ (FAQ, ПРОМО, СЕРТИФИКАТЫ, СЛОТЫ, КАМНИ, ВОПРОСЫ)
# ======================================================
...
async def admin_list_stones(cb: CallbackQuery):
    ...
    with db_cursor() as c:
        c.execute("SELECT id, stone_name, emoji FROM stones_knowledge ORDER BY stone_name LIMIT 31")
        stones = c.fetchall()
...
# ======================================================
# АДМИН - ФОРМЫ ДОБАВЛЕНИЯ (Требуют FSM, минимальная реализация)
# ======================================================
...
# ═══════════════════════════════════════════════════════════════════════════
# КОНЕЦ ФИНАЛЬНЫХ HANDLERS
# ═══════════════════════════════════════════════════════════════════════════

# ═══════════════════════════════════════════════════════════════════════════
# ПОСЛЕДНИЕ 11 HANDLERS ДЛЯ 100% ЗАВЕРШЁННОСТИ
# ═══════════════════════════════════════════════════════════════════════════
...
async def main():
    ...
    # ✅ FIX: Middleware регистрируется ДО include_router — иначе не применяется корректно
...
async def confirm_bonus_payment(cb: CallbackQuery):
    ...
    # ✅ FIX: флаги результата вместо await внутри with
    _insufficient = False
    _error_msg = None
    _order_id = None
    _remaining = None
    ...
    with db_connection() as conn:
        ...
        try:
            ...
            if balance <= 0:
                _insufficient = True  # ✅ не await внутри with
            else:
    ...
    # ✅ FIX: все await строго после закрытия with — нет deadlock с BEGIN IMMEDIATE
    if _insufficient:
    ...
    text = "✅ *ЗАКАЗ ОФОРМЛЕН*\n\n"
    text += f"Номер заказа: #{_order_id}\n"
    text += f"Списано бонусов: {int(_bonus_used)}\n"
    if _remaining > 0:
        text += f"\n⏳ Осталось оплатить: {int(_remaining)}₽\n"
